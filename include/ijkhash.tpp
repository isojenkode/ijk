...
/*
  ...
  Copyright (C) 2018-2025 Rephael Wenger
...
namespace IJK {
  ...
  // *****************************************************************
  // Class HASH_PAIR for hashing keys of type pair
  // *****************************************************************
  ...
  // *****************************************************************
  // Hash table of bitset
  // *****************************************************************

  /*!
   *  @brief Insert or update bitset data in hash table.
   *  - Set hash_table[key].bitset[iloc] to 1.
   *  @param facet_vertex Index of lower/leftmost facet vertex (map key).
   *  @param iloc Bit location.
   *    @pre 0 <= iloc < bitset.size.
   *  @param ival Bit value. 0 or 1.
   */
  template <typename KEY_TYPE, typename BITSET_TYPE,
            typename ITYPE, typename VTYPE>
  void insert_or_update_hash_table_bitset
  (const KEY_TYPE key,
   const ITYPE iloc,
   const VTYPE ival,
   std::unordered_map<KEY_TYPE,BITSET_TYPE> & hash_table)
  {
    auto htable_iter = hash_table.find(key);
    if (htable_iter != hash_table.end()) {
      (htable_iter->second)[iloc] = ival;
    }
    else {
      BITSET_TYPE bitset;
      bitset[iloc] = ival;
      hash_table.insert(std::pair(key, bitset));
    }
  }
