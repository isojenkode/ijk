/*!
 *  @file ijkcontourtree.tpp
 *  @brief ijk templates defining contour tree routines and data structures.
 * - Version 0.6.0
 */
...
namespace IJK {
  ...
  namespace CONTOUR_TREE {
    ...
    template <const int CUBEV_BITSET_SIZE,
              typename DTYPE, typename ATYPE, typename VTYPE, 
	      typename NTYPE, typename BTYPE>
    class VERTEX_NEIGHBORS:
    ...
    template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
              typename VTYPE, typename NTYPE, typename BTYPE>
    void VERTEX_NEIGHBORS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE,BTYPE>::
    ...
    template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
              typename VTYPE, typename NTYPE, typename BTYPE>
    VERTEX_NEIGHBORS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE,BTYPE>::
    VERTEX_NEIGHBORS(const DTYPE dimension, const ATYPE * axis_size):
    ...
    namespace {
       ...
       COMPARE_FUNC compare)
      {
        ...
        compute_coord(iv, dimension, axis_size, coord);
       ...
       const VTYPE iv, VTYPE2 * vlist, NUM_TYPE & num_vertices)
      {
        ...
        VTYPE subgrid_origin = 
          ...
          (subgrid_origin_coord, dimension, axis_size);

        IJK::compute_num_grid_vertices
          (dimension, subgrid_axis_size, num_vertices);
      ...
      template<const int CUBEV_BITSET_SIZE, typename NUM_TYPE,
               typename DTYPE, typename ATYPE, typename VTYPE,
      ...
      (const NUM_TYPE i, 
       const VTYPE * sorted_vertices, const LTYPE * vertex_loc,
       VERTEX_NEIGHBORS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NUM_TYPE,BTYPE> &
       neighbors,
       NODE_TYPE * tree, UNION_FIND<NUM_TYPE,NUM_TYPE> & set,
       COMPARE_FUNC compare)
      {
      ...
      (const NUM_TYPE i, 
       const VTYPE * sorted_vertices, const LTYPE * vertex_loc,
       const BGRID_TYPE & boundary_grid,
       VERTEX_NEIGHBORS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NUM_TYPE,BBITS_TYPE> &
       neighbors,
       NODE_TYPE * tree, UNION_FIND<NUM_TYPE,NUM_TYPE> & set,
       COMPARE_FUNC compare)
      {
       ...
       const VTYPE iv, VTYPE2 * vlist, NUM_TYPE & num_vertices)
      {
        ...
        for (NUM_TYPE i = 0; i < num_cube_connected; i++) {
          ...
          compute_coord(iv2, dimension, axis_size, coord2);
    ...
    /// @pre \li Array tree[] is preallocated to length at least \a num_vertices.
    template <const int CUBEV_BITSET_SIZE,
              typename DTYPE, typename ATYPE, typename VTYPE, typename LTYPE, 
     ...
     NODE_TYPE * tree)
    {
      ...
      VERTEX_NEIGHBORS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NUM_TYPE,long> 
      ...
      IJK::compute_increment(dimension, axis_size, axis_increment);
      ...
      case(EDGE_CONNECT):
        for (NUM_TYPE i = 0; i < num_vertices; i++) {
          add_vertex_edge_connect
            (i, sorted_vertices, vertex_loc, boundary_grid, axis_increment,
        ...
        for (NUM_TYPE i = 0; i < num_vertices; i++) {
          add_vertex_facet_connect
            (i, dimension, axis_size, sorted_vertices, vertex_loc,
             axis_increment, tree, set, join_compare);
        }
        break;
        ...
        for (NUM_TYPE i = 0; i < num_vertices; i++) {
          add_vertex_edge_connect
            (num_vertices-1-i, sorted_vertices, vertex_loc, boundary_grid,
             axis_increment, tree, set, split_compare);
        }
        break;
    ...
    template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
              typename NUM_TYPE, typename VTYPE, typename LTYPE,
     ...
     CNODE_TYPE * contour_tree)
    {
      ...
      construct_join_tree<CUBEV_BITSET_SIZE>
        (dimension, axis_size, sorted_vertices, vertex_loc,
         num_vertices, join_connectivity, join_tree);

      construct_split_tree<CUBEV_BITSET_SIZE>
        (dimension, axis_size, sorted_vertices, vertex_loc,
    ...
    /*!
     *  @brief Construct a join tree from a regular grid.
     *  @param dimension = Grid dimension.
     *  @param axis_size[d] = Number of vertices along grid axis d.
     *    @pre \li Array tree[] is preallocated to length at least \a num_vertices.
     */
    template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
              typename STYPE, typename NODE_TYPE>
    void construct_join_tree
    (const DTYPE dimension, const ATYPE * axis_size, const STYPE * scalar,
    ...
    /*!
     *  @brief Construct a contour tree.
     *  @tparam CNODE_TYPE Contour tree node type.
     *    - Must include member function Ident() and SetIdent().
     */
    template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
              typename STYPE, typename CNODE_TYPE>
    void construct_contour_tree
    (const DTYPE dimension, const ATYPE axis_size, const STYPE * scalar,
     const CONNECTIVITY join_connectivity,
     const CONNECTIVITY split_connectivity,
     ...
     const MERGE_IDENT_FLAG merge_ident_flag = MERGE_IDENT_CTREE)
    {
      ...
      if (augment_flag == AUGMENT_ALL && 
        ...
        construct_augmented_contour_tree<CUBEV_BITSET_SIZE>
          (dimension, axis_size, num_vertices,
           sorted_vertices.PtrConst(), vertex_loc.PtrConst(),
           augmented_jtree.Ptr(), join_connectivity, 
           augmented_stree.Ptr(), split_connectivity, 
     ...
     *    - Must include member function Ident() and SetIdent().
     */
    template <const int CUBEV_BITSET_SIZE, typename GTYPE,
              typename CNODE_TYPE>
    void construct_contour_tree
    (const GTYPE & grid,
     const CONNECTIVITY join_connectivity,
     ...
     const MERGE_IDENT_FLAG merge_ident_flag = MERGE_IDENT_CTREE)
    {
      construct_contour_tree<CUBEV_BITSET_SIZE>
        (grid.Dimension(), grid.AxisSize(), grid.ScalarPtrConst(),
    ...
    (const NTYPE e1, const NTYPE e2, const DTYPE d)
    {
      ...
      NTYPE new_root = this->FindRoot(e1);
      data[new_root] = d;
    }
