...
namespace IJK {
  ...
  inline int cube_facet_orth_dir(const int dimension, const FTYPE kf)
  {
    return (kf%dimension);
  }
   ...
   */
  template <typename FTYPE>
  inline int cube_facet_side(const int dimension, const FTYPE kf)
  {
    return (kf/dimension);
  }
  ...
  /*!
   *  @brief Return index of facet on opposite side of cube.
   *  - Return 0 if facet contains vertex 0.
   *  - Return 1 if facet does not contain vertex 0.
   */
  template <typename FTYPE>
  inline int opposite_cube_facet(const int dimension, const FTYPE kf)
  {
    return ((kf+dimension)%(2*dimension));
  }
  ...
  /*!
   *  @brief Base cube class.
   *  - Vertex indices determine vertex locations in cube.
   *  - Vertex iv is on lower/leftmost cube facet orthogonal to direction d
   *    if d'th bit of iv is 0.
   *  - Vertex iv is on upper/rightmost cube facet orthogonal to direction d
   *    if d'th bit of iv is 1.
   */
  ...
  public:
    ...
    /*!
     *  @brief Return side (0 or 1) of cube containing vertex.
     *  - Side is orthogonal to directio orth_dir.
     */
    int SideContainingVertex
    (const NTYPE iv, const DTYPE orth_dir) const
    {
      const int mask = (1L << orth_dir);
      const int mask2 = (iv & mask);
      if (mask2 == 0) { return 0; }
      else { return 1; }
    }
