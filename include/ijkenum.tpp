...
namespace IJK {
  ...
  public:
    ...
    /// @oveload
    /// @brief Return enum value named by enum_str. (const char * string)
    /// - Version where enum_str is const char * string.
    /// - All cases ignored (converted to lower case).
    ENUM_TYPE EnumValue(const char * enum_str) const
    { return EnumValue(std::string(enum_str)); }
    ...
    /// @brief Print all strings (mainly for debugging.)
    template <typename OSTREAM_TYPE>
    void PrintAllStrings(OSTREAM_TYPE & out) const;

    /// @overload
    /// @brief Print all strings and prefix/suffix strings (mainly for debugging.)
    /// - Print prefix and suffix strings.
    template <typename OSTREAM_TYPE>
  ...
  template <typename _ENUM_TYPE>
  template <typename OSTREAM_TYPE>
  void ENUM_LIST<_ENUM_TYPE>::
  PrintAllStrings(OSTREAM_TYPE & out) const
  {
    for (int i = 0; i < NumEnum(); i++) {
      out << "\"" << String(ENUM_TYPE(i)) << "\"";
      if (i+1 != NumEnum()) { out << ","; }
    }
  }

  
  // @brief Print all strings and prefix/suffix strings.
  template <typename _ENUM_TYPE>
  template <typename OSTREAM_TYPE>
  void ENUM_LIST<_ENUM_TYPE>::
  PrintAllStrings
