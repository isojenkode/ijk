/*!
 *  @file ijkmerge.tpp
 *  @brief Data structures to merge identical values in lists.
 *  - Version 0.6.0
 */
...
namespace IJK {
  ...
  /*!
   *  @brief Base class for list of integers with no duplicate entries.
   *  - Integers must be in a predefined range of size \a max_num_int.
   *  - Allocates storage of length \a max_num_int.
   */
  template <typename ITYPE, typename LTYPE> 
  class INTEGER_LIST {

  public:
    typedef ITYPE INTEGER_TYPE;
    typedef LTYPE LOC_TYPE;
  ...
  protected:
    ...
    /// @brief Maximum number of integers that can be inserted.
    ...
    void Init(const INTEGER_TYPE min_int, const LOC_TYPE max_num_int);
    void Init(const LOC_TYPE max_num_int) { Init(0, max_num_int); };
    void FreeListLoc();
  ...
  public:
    ...
    /// @brief Constructor.
    /// - INTEGER_LIST allocates and initializes memory of size max_num_int.
    INTEGER_LIST(const INTEGER_TYPE min_int,
                 const LOC_TYPE max_num_int) 
    { Init(min_int, max_num_int); };

    /// @brief Constructor.
    /// - Version that initializes min_int to 0.
    INTEGER_LIST(const LOC_TYPE max_num_int) { Init(max_num_int);};
    ...
    /// @brief Return true if (MinInt() <= i < MinInt()+MaxNumInt()).
    bool InRange(const INTEGER_TYPE i) const
    {
      if (i < MinInt()) { return false; }
      if (i >= MinInt()+MaxNumInt()) { return false; }
      return true;
    }
    ...
    /// @brief Insert integer i into list.
    /// @pre min_int <= i < min_int+max_num_int.
    ...
    /// Return false and set error message if list fails check.
    bool CheckList(IJK::ERROR & error) const;
  
  };


  /*!
   *  @brief Initialize INTEGER_LIST.
   *  @param min_int Minimum value of any integer in list.
   *  @param max_num_int Maximum number of integers that can be inserted.
   */
  template < class ITYPE, class LTYPE> 
  void INTEGER_LIST<ITYPE,LTYPE>::
  Init(const ITYPE min_int, const LTYPE max_num_int)
  {
  ...
  /// Free list loc.
  template <class ITYPE, class LTYPE> 
  void INTEGER_LIST<ITYPE,LTYPE>::FreeListLoc()
  {
    delete [] list_loc;
    list_loc = NULL;
    delete [] in_list;
  ...
  /// @brief Reset min integer.
  /// @param min_int Minimum value of any integer in list.
  template < class ITYPE, class LTYPE> 
  void INTEGER_LIST<ITYPE,LTYPE>::
  ResetMin(const ITYPE new_min_int)
  {
    ClearList();
  ...
  // Check list.
  template <class ITYPE, class LTYPE> 
  bool INTEGER_LIST<ITYPE,LTYPE>::
  CheckList(IJK::ERROR & error) const
  {
    for (LOC_TYPE i = 0; i < ListLength(); i++) {
      const INTEGER_TYPE ival = List(i);
      if (!InRange(ival)) {
        error.AddMessage
          ("list[", i, "]=", ival, " is not in range ",
           "[", MinInt(), ",", MinInt()+MaxNumInt(), ").");
        return false;
      }
      ...
      if (!InList(ival)) {
        error.AddMessage
          ("list[", i, "]=", ival,
           ", but InList(", ival, ") is false.");
        return false;
      }

      const LOC_TYPE jloc = ListLoc(ival);
      if (jloc != i) {
        error.AddMessage
          ("list[", i, "]=", ival,
           ", but ListLoc(", ival, ")=", jloc, ".");
      }
    }

    for (INTEGER_TYPE ival = MinInt(); ival < MinInt()+MaxNumInt(); ival++) {
      if (InList(ival)) {
        const LOC_TYPE jloc = ListLoc(ival);
        if (List(jloc) != ival) {
          error.AddMessage
            ("ListLoc(", ival, ")=", jloc,
             " but List(", jloc, ")=", List(jloc), ".");
          return false;
        }
      }
    }
  ...
  /*!
   *  @brief Initialize INTEGER_COUNT.
   *  @param min_int Minimum value of any integer in list.
   *  @param max_num_int Maximum number of integers that can be inserted.
   *  @pre max_num_int is non-negative.
   */
  template < class INTEGER_TYPE, class LOC_TYPE, class NTYPE> 
   ...
   MTYPE * list0_map, INTEGER_LIST_TYPE & int_list)
  {
    ...
    for (NTYPE i = 0; i < NTYPE(int_list.ListLength()); i++)
  ...
  /*!
   *  @overload
   *  @brief Merge identical values in an integer list. (C++ vector.)
   *  - Version using C++ STL vector.
   *  @param list0 List of integers.
   */
  ...
  /// @brief Comparison function template.
  ...
  /*!
   *  @brief Separate array of pairs into array of first and array of second.
   *  @param first_list = array of first entries of pair.
   *  @pre first_list is preallocated to length at least num_pair.
   *  @param second_list = array of second entries of pair.
   *  @pre second_list is preallocated to length at least num_pair.
   */
  ...
  /*!
   *  @brief Reorder list so that identical data entries are clustered together
   *  @param data[] Values of elements in list[].
   *  @param list[] List of items (locations) in data[].
   *    - Value associated with list[i] is data[list[i]].
   *  @param list_length Number of elements in list[].
   *  @param[out] list2[] Reordered list.
   *  @param icount Integer count list data structure
   *  @pre icount.MinInt() <= min data element in list.
   *  @pre icount.MinInt()+icount.NumInt() > max data element in list.
   */
  ...
  /*!
   *  @brief Merge pairs using bins.
   *  @param log2_interval log base 2 of interval covered by each bin.
   *  @pre Precondition: log2_interval >= 0.
   */
  ...
  /// @brief Check for error in pair_list_nodup and pair_list_loc.
