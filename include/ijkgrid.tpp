...
namespace IJK {
  ...
  public:
    ...
    VTYPE ComputeVertexIndex(const CTYPE * coord) const;
    template <typename CTYPE>
    VTYPE ComputeVertexIndex(const std::vector<CTYPE> coord) const;
    template <typename CTYPE>
    void ComputeCoord(const VTYPE iv, CTYPE * coord) const;
    template <typename CTYPE>
    void ComputeCubeCenterCoord(const VTYPE iv, CTYPE * coord) const;
    template <typename CTYPE>
    void ComputeEdgeMidpoint
    (const VTYPE iend0, const int edge_direction, CTYPE * coord) const;
    ...
    template <typename CTYPE>
    bool CheckCoord(const CTYPE * coord, IJK::ERROR & error) const;
    template <typename CTYPE>
    bool CheckCoord(const std::vector<CTYPE> & coord, 
                    IJK::ERROR & error) const;

    /// Return true if coord is the coordinate of a grid cube.
    template <typename CTYPE>
    bool CheckCubeCoord(const CTYPE * coord, IJK::ERROR & error) const;
    template <typename CTYPE>
    bool CheckCubeCoord(const std::vector<CTYPE> & coord, 
                    IJK::ERROR & error) const;
   ...
   *  @tparam CUBEV_BITSET_SIZE Size of bitset storing
   *    cube vertex locations.
   *    - Almost always 8.
   *  @pre CUBEV_BITSET_SIZE >= Dimension().
   *  @tparam DTYPE  Dimension data type.
   *  @tparam ATYPE  Axis size type.
   *  @tparam VTYPE  Vertex index type.
   *  @tparam NTYPE  Number type.
   */
  template <const int CUBEV_BITSET_SIZE,
            typename DTYPE, typename ATYPE, typename VTYPE, typename NTYPE> 
  ...
  public:
    ...
    typedef std::bitset<CUBEV_BITSET_SIZE> CUBEV_BITSET_TYPE;
  ...
  protected:
    ...
    /*!
     *  \brief Orientation of facet 0, (facet in hyperplane x_0 = 0.
    ...
    /*!
     *  @brief cube_vertex_bitset[i] = Bitset representing location
     *    of cube vertex i.
     *  - If d'th bit is 0, then vertex is on lower/leftmost
     *    facet orthogonal to direction d.
     *  - If d'th bit is 1, then vertex is on upper/rightmost
     *    facet orthogonal to direction d.
     *  - In unit cube, d'th bit equals d'th vertex coordinate.
     */
    CUBEV_BITSET_TYPE * cube_vertex_bitset;
    
    /// DEPRECATED: unit_cube_coord[dimension*k+j] = j'th coordinate of k'th vertex of unit cube
  ...
  public:
    ...
    GRID_PLUS(const GRID_PLUS
              <CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE> & grid2);
    ...
    /*!
     *  @brief Return location of vertex j in unit cube.
     *  - If 0, then vertex is on lower/leftmost
     *    facet orthogonal to direction d.
     *  - If 1, then vertex is on upper/rightmost
     *    facet orthogonal to direction d.
     */
    int UnitCubeVertexLocation(const int j, const int d) const
    { return cube_vertex_bitset[j][d]; }

    /// Return cube_vertex_bitset[j].
    CUBEV_BITSET_TYPE CubeVertexBitset(const int j) const
    { return cube_vertex_bitset[j]; }

    /// Return string representing cube_vertex_bitset[j].
    std::string CubeVertexBitsetString(const int j) const;
    
    // *** DEPRECATED. REPLACE BY class UnitCubeVertexLocation ***
    ...
    /// @brief Return true if dataset passes all checks.
    ...
    bool Check(const GRID<DTYPE2,ATYPE2,VTYPE2,NTYPE2> & grid,
               IJK::ERROR & error) const
    {
      return GRID<DTYPE2,ATYPE2,VTYPE2,NTYPE2>::Check(grid, error);
    }
    ...
    bool Check(const GRID<DTYPE2,ATYPE2,VTYPE2,NTYPE2> & grid2,
               ...
               IJK::ERROR & error) const
    {
      return GRID<DTYPE2,ATYPE2,VTYPE2,NTYPE2>::
	Check(grid2, grid_label1, grid_label2, error);
    }

    
    /// @brief Return true if array cube_vertex_bitset[] passes check.
    bool CheckCubeVertexBitset(IJK::ERROR & error) const;
  };
   ...
   *  @tparam NTYPE  Number type.
   */
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename DIFFTYPE, typename NTYPE> 
  class GRID_NEIGHBORS:
    public GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE> {
  ...
  public:
    ...
    (const GRID_NEIGHBORS
     <CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,DIFFTYPE, NTYPE> & grid2);
  ...
  public:
    ...
    /// Return true if grid *this has array spacing allocated.
    bool CheckSpacing(IJK::ERROR & error) const;
   ...
   *  - GEOM_GRID has both origin and grid spacing.
   *  @tparam GRID_TYPE Type of grid base.
   *  - Note: GEOM_GRID is defined to always be a subclass
   *    of GRID_SPACING.
   *  - GRID_TYPE should not be a class or subclass of GRID_SPACING.
   */
  ...
  ComputeVertexIndex(const CTYPE * coord) const
  {
  ...
  template <typename CTYPE>
  VTYPE GRID<DTYPE,ATYPE,VTYPE,NTYPE>::
  ComputeVertexIndex(const std::vector<CTYPE> coord) const
  {
  ...
  ComputeCoord(const VTYPE iv, CTYPE * coord) const
  {
  ...
  template <typename CTYPE>
  void GRID<DTYPE,ATYPE,VTYPE,NTYPE>::
  ComputeCubeCenterCoord(const VTYPE iv, CTYPE * coord) const
  {
  ...
  /*!
   *  @brief Compute midpoint of edge.
   *  @param[out] midpoint_coord[d] d'th coordinate of midpoint.
   *  @pre Array <em>midpoint_coord[]</em> is preallocated to length
   *       at least \a Dimension().
   */
  template <typename DTYPE, typename ATYPE, typename VTYPE, typename NTYPE>
  template <typename CTYPE>  
  void GRID<DTYPE,ATYPE,VTYPE,NTYPE>::
  ComputeEdgeMidpoint(const VTYPE iend0, const int edge_direction,
                      CTYPE * midpoint_coord) const
  {
    this->ComputeCoord(iend0, midpoint_coord);
    midpoint_coord[edge_direction] += 0.5;
  }
  ...
  template <typename CTYPE>
  bool GRID<DTYPE,ATYPE,VTYPE,NTYPE>::
  CheckCoord(const CTYPE * coord, IJK::ERROR & error) const
  {
    for (DTYPE d = 0; d < Dimension(); d++) {
      if (coord[d] < 0) {
  ...
  template <typename CTYPE>
  bool GRID<DTYPE,ATYPE,VTYPE,NTYPE>::
  CheckCoord(const std::vector<CTYPE> & coord, IJK::ERROR & error) const
  {
  ...
  template <typename CTYPE>
  bool GRID<DTYPE,ATYPE,VTYPE,NTYPE>::
  CheckCubeCoord(const CTYPE * coord, IJK::ERROR & error) const
  {
    for (DTYPE d = 0; d < Dimension(); d++) {
      if (coord[d] < 0) {
  ...
  template <typename CTYPE>
  bool GRID<DTYPE,ATYPE,VTYPE,NTYPE>::
  CheckCubeCoord(const std::vector<CTYPE> & coord, IJK::ERROR & error) const
  {
  ...
  /// Constructor.
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE> 
  GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::GRID_PLUS
  (const DTYPE dimension, const ATYPE * axis_size):
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE> 
  template <typename DTYPE2, typename ATYPE2, typename VTYPE2, typename NTYPE2>
  GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::
  GRID_PLUS(const GRID<DTYPE2,ATYPE2,VTYPE2,NTYPE2> & grid2):
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE> 
  void GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::ZeroLocal()
  {
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE> 
  void GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::InitLocal()
  {
    ZeroLocal();
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE> 
  void GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::Create()
  {
    const DTYPE dimension = this->Dimension();
    ...
    if (!IJK::check_is_NULL
      ...
      { throw error; }
    if (!IJK::check_is_NULL
        (cube_vertex_bitset, "cube_vertex_bitset", error))
      { throw error; }
    ...
    if (num_cube_vertices > 0) {
      ...
      this->cube_vertex_bitset = new CUBEV_BITSET_TYPE[num_cube_vertices];
      compute_cube_vertex_bitset(dimension, this->cube_vertex_bitset);
    }
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE> 
  void GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::FreeLocal()
  {
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE> 
  template <typename DTYPE2, typename ATYPE2>
  void GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::SetSize
  (const DTYPE2 dimension, const ATYPE2 * axis_size)
  {
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE> 
  template <typename VTYPE2, typename VTYPE3>
  DTYPE GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::EdgeDirection
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE> 
  template <typename VTYPE0, typename VTYPE1, typename DIST_TYPE>
  void GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::
  GetVertexNeighbors(const VTYPE0 iv0, const DIST_TYPE distance,
                     std::vector<VTYPE1> & vlist) const
  {
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE> 
  template <typename ITYPE>
  NTYPE GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE>
  VTYPE GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE>
  std::string GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::
  CubeVertexBitsetString(const int j) const
  {
    const std::string s = CubeVertexBitset(j).to_string();
    const int L = s.length();

    return s.substr(L-this->Dimension());
  }
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE>
  bool GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::
  Check(IJK::ERROR & error) const
  {
    if (!CheckCubeVertexBitset(error))
      { return false; }
  ...
  // Return true if array cube_vertex_bitset[] passes check.
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename NTYPE>
  bool GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE>::
  CheckCubeVertexBitset(IJK::ERROR & error) const
  {    
    ...
    if (!IJK::check_array_allocated
        (cube_vertex_bitset, "GRID_PLUS::cube_vertex_bitset", error))
      { return false; }

    // Check that vertex location indicated by cube_vertex_bitset[j]
    //   is consistent with cube_vertex_increment[j].
    for (int j = 0; j < this->NumCubeVertices(); j++) {
      const VTYPE cubev_increment_j =
        compute_cube_vertex_j_increment
        (dimension, CubeVertexBitset(j), AxisIncrement());
      if (cubev_increment_j != CubeVertexIncrement(j)) {
        error.AddMessage
          ("Programming error. Cube vertex ", j,
           " bitset and increment are inconsistent.");
        error.AddMessage
          ("  Vertex increment: ", CubeVertexIncrement(j));
        error.AddMessage
          ("  Cube vertex bitset: ", CubeVertexBitsetString(j));
        error.AddMessage
          ("  Increment based on bitset: ", cubev_increment_j);
        return false;
      }
    }


    // Check that vertex location indicated by cube_vertex_bitset[]
    //   is consistent with facet_vertex_increment[].

    const int num_cube_vertices = this->NumCubeVertices();

    for (int jv = 0; jv < num_cube_vertices; jv++) {
      const VTYPE increment_jv = CubeVertexIncrement(jv);
      for (int d = 0; d < dimension; d++) {
        const int iside = UnitCubeVertexLocation(jv, d);
        const int ifacet = this->CubeFacetIndex(d, bool(1-iside));

        bool found = false;
        for (int k = 0; k < this->NumCubeFacetVertices(); k++) {
          if (FacetVertexIncrement(ifacet, k) == increment_jv) {
            found = true;
            break;
          }
        }
        ...
        if (!found) {
          error.AddMessage
              ("Programming error. Cube and facet vertex increment arrays are inconsistent.");
            error.AddMessage
              ("  Cube vertex ", jv, " bitset indicates vertex ", jv,
               " is on facet ", ifacet, ".");
            error.AddMessage
              ("  However, facet ", ifacet,
               " has no vertex increment equal to cube vertex ", jv, " increment.");
            error.AddMessage
              ("  Facet ", ifacet, " is orthogonal to direction ", d,
               " and on cube side ", iside, ".");
            error.AddMessage
              ("  Cube vertex ", jv, " increment: ", increment_jv);
            return false;
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename DIFFTYPE, typename NTYPE> 
  GRID_NEIGHBORS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,DIFFTYPE,NTYPE>::
  GRID_NEIGHBORS
  (const DTYPE dimension, const ATYPE * axis_size):
    GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE> (dimension,axis_size)
  {
    InitLocal();
  }
  ...
  GRID_NEIGHBORS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,DIFFTYPE,NTYPE>::
  GRID_NEIGHBORS(const GRID<DTYPE2,ATYPE2,VTYPE2,NTYPE2> & grid2):
    GRID_PLUS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,NTYPE> (grid2)
  {
    InitLocal();
  }
  ...
  template <const int CUBEV_BITSET_SIZE, typename DTYPE, typename ATYPE,
            typename VTYPE, typename DIFFTYPE, typename NTYPE> 
  void GRID_NEIGHBORS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,DIFFTYPE,NTYPE>::
  ZeroLocal()
  {
  ...
  void GRID_NEIGHBORS<CUBEV_BITSET_SIZE,DTYPE,ATYPE,VTYPE,DIFFTYPE,NTYPE>::
  SetSize(const GRID<DTYPE2,ATYPE2,VTYPE2,NTYPE2> & grid2)
  {
    SetSize(grid2.Dimension(), grid2.AxisSize());
  }
  ...
  {
    IJK::PROCEDURE_ERROR error("GRID_SPACING::SetAllSpacing");

    if (!CheckSpacing(error)) { throw error; }
  ...
  {
    IJK::PROCEDURE_ERROR error("GRID_SPACING::SetSpacing");
  ...
  // Return true if grid *this has array spacing allocated.
  template <typename SP_TYPE, typename GRID_TYPE>
  bool GRID_SPACING<SP_TYPE, GRID_TYPE>::
  CheckSpacing(IJK::ERROR & error) const
  {
    if (this->Dimension() > 0) {
      if (!IJK::check_array_allocated(spacing, "spacing", error)) {
        error.AddMessage("  dimension = ", this->Dimension());
        return false;
      }
    }
  ...
  (const DTYPE2 dimension, const ATYPE2 * axis_size)
  {
    ...
    // Note: GEOM_GRID is defined to always be a subclass
    //   of GRID_SPACING.
    GRID_SPACING<CTYPE,GRID_TYPE>::SetSize(dimension, axis_size);
