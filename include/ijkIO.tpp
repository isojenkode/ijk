...
namespace IJK {
   ...
   *     in the same class.
   */
  template <typename _COORD_TYPE, typename _OTHER_DATA>
  class OUTPUT_PARAM_BASE:public _OTHER_DATA {
  ...
  public:
    ...
    /// Dimension of vertex coordinates.
    int dimension;
    ...
    std::string write_isov_filename;
    ...
    /// Output precision.
    IJK::SET_VALUE<int> output_precision;
  ...
  public:
    ...
    /// Constructor.
    OUTPUT_PARAM_BASE():output_precision(0) { Init(); };
  ...
  public:
    ...
    /// Return dimension of vertex coordinates.
    int Dimension() const
    { return dimension; }
    ...
    /// Print OUTPUT_PARAM_BASE information. (May not be complete.)
  ...
  /*!
   *  @brief Write mesh with output format output_format to output out.
   ...
   const std::vector<std::string> & comment)
  {
    ...
    case OUTPUT_DEFINITIONS::OFF:
      if (mesh_dimension == DIM2 &&
        ...
        ijkoutQuadOFF(out, dimension, vertex_coord, plist,
                      flag_reorder_quad_vertices, comment);
      }
      else {
        ijkoutOFF(out, dimension, num_vertices_per_isopoly,
    ...
    case OUTPUT_DEFINITIONS::PLY:
      if (mesh_dimension == DIM2 && dimension == DIM3) {
        if (num_vertices_per_isopoly == NUM_VERT_PER_QUAD) {
          ijkoutQuadPLY(out, dimension, vertex_coord, plist,
                        flag_reorder_quad_vertices, comment);
        }
        else{
          ijkoutPLY
            (out, dimension, num_vertices_per_isopoly,
             vertex_coord, plist, comment);
        }
      }
      else throw error
    ...
    case OUTPUT_DEFINITIONS::FIG:
      if (mesh_dimension == DIM1 && dimension == DIM2) {
        ijkoutFIG(out, dimension, vertex_coord,
   ...
   const std::vector<std::string> & comment)
  {
    ...
    if (flag_use_stdout) {
      write_mesh(std::cout, output_info, output_format, vertex_coord, plist, comment);
    }
    else {
      ...
      if (ofilename == "") {
        throw error("Missing output filename.");
      }

      output_file.open(ofilename.c_str(), std::ios::out);

      if (!output_file.is_open()) {
        error.AddMessage("Unable to open file ", ofilename, ".");
        error.AddMessage("  Output write failed.");
        throw error;
      }

      if (output_info.output_precision.IsSet()) {
        output_file.precision(output_info.output_precision.Value());
      }
      
      write_mesh(output_file, output_info, output_format,
                 vertex_coord, plist, comment);
  ...
  // *****************************************************************
  // OUTPUT_PARAM_BASE MEMBER FUNCTIONS
  // *****************************************************************

  // Initialize ISO_IO_PARAM_BASE.
  template <typename _COORD_TYPE, typename _OTHER_DATA>
  void OUTPUT_PARAM_BASE<_COORD_TYPE,_OTHER_DATA>::Init()
  {
    const int DIM2(2);
    const int DIM3(3);
  ...
  template <typename _COORD_TYPE, typename _OTHER_DATA>
  int OUTPUT_PARAM_BASE<_COORD_TYPE,_OTHER_DATA>::
  ...
  template <typename _COORD_TYPE, typename _OTHER_DATA>
  void OUTPUT_PARAM_BASE<_COORD_TYPE,_OTHER_DATA>::  
  SetOutputFormat(const OUTPUT_FORMAT output_format)
  {
    IJK::PROCEDURE_ERROR error("ISO_IO_PARAM_BASE::SetOutputFormat");
  ...
  {
    IJK::PROCEDURE_ERROR
      error("OUTPUT_PARAM_BASE::SetOutputFilename");
  ...
  (const OUTPUT_FORMAT output_format, const char * output_filename)
  {
    IJK::PROCEDURE_ERROR error("ISO_IO_PARAM_BASE::SetOutputFilename");
  ...
  /// Print OUTPUT_PARAM_BASE information. (Not complete.)
  template <typename _COORD_TYPE, typename _OTHER_DATA>
  template <typename OSTREAM_TYPE, typename STYPE0>
  void OUTPUT_PARAM_BASE<_COORD_TYPE,_OTHER_DATA>::  
  Print(OSTREAM_TYPE & out, const STYPE0 & line_prefix) const
  {
    ...
    out << line_prefix << "flag_output_fig: "
        << IJK::bool2string(flag_output_fig) << "\n";
