...
/*
  ...
  Copyright (C) 2015-2025 Rephael Wenger
...
namespace IJK {
  ...
  // *******************************************************************
  //! @name Compute intersections of isosurface and line segments.
  // *******************************************************************
   ...
   *         using linear interpolation. (Edge=(iend0,direction).)
   *  - Grid edge specified by lower/left endpoint and edge direction.
   *  @param scalar_grid Scalar grid data.
   *  @param iend0 Lower/left edge endpoint.
   *  @param edge_direction Axis parallel edge direction. (Integer.)
   *  @param s0 Scalar value at iend0.
   *  @param s1 Scalar value at other endpoint of grid edge.
   *  @param[out] isov_coord[] Array of isosurface vertex coordinates.
   *    - isov_coord[d] = d'th coordinate of isosurface vertex.
   *  @pre Array isov_coord[] is preallocated to length at least dimension.
   *  @param[out] c0 Coordinate edge_direction of iend0.
   *  @param[out] c1 Coordinate edge_direction of iend1
   *    (upper/right edge endpoint).
   */
  template <typename GRID_TYPE, typename ISOVALUE_TYPE,
            typename STYPE0, typename STYPE1,
            typename VTYPE, typename CTYPEV, typename CTYPEG>
  inline void compute_isosurface_grid_edge_intersection_linear
  (const GRID_TYPE & grid, const ISOVALUE_TYPE isovalue,
   const VTYPE iend0, int edge_direction,
   const STYPE0 s0, const STYPE1 s1, CTYPEV isov_coord[],
   CTYPEG & c0, CTYPEG & c1)
  {
    // Isosurface vertex has exact same coordinates as grid vertex iend0,
    // except for isov_coord[edge_direction].
    grid.ComputeCoord(iend0, isov_coord);

    // Compute isov_coord[edge_direction].
    const CTYPEV w0 =
      compute_linear_interpolation_coef<CTYPEV>(s0, s1, isovalue);
    const CTYPEV w1 = 1 - w0;

    c0 = isov_coord[edge_direction];
    c1 = c0 + 1;
    isov_coord[edge_direction] = w0*c0 + w1*c1;
  }
   ...
   *  @overload
   *  @brief Compute intersection of isosurface and grid edge
   *         using linear interpolation. (Does not return c0 or c1.)
   *  - (Edge=(iend0,direction).)
   *  - Grid edge specified by lower/left endpoint and edge direction.
   *  @param scalar_grid Scalar grid data.
   *  @param iend0 Lower/left edge endpoint.
   *  @param edge_direction Axis parallel edge direction. (Integer.)
  ...
  template <typename SGRID_TYPE, typename ISOVALUE_TYPE,
            typename STYPE0, typename STYPE1,
            typename VTYPE, typename CTYPEV>
  inline void compute_isosurface_grid_edge_intersection_linear
  (const SGRID_TYPE & scalar_grid, const ISOVALUE_TYPE isovalue,
   const VTYPE iend0, int edge_direction,
   const STYPE0 s0, const STYPE1 s1,
   CTYPEV isov_coord[])
  {
    CTYPEV c0, c1;

    compute_isosurface_grid_edge_intersection_linear
      (scalar_grid, isovalue, iend0, edge_direction, s0, s1,
       isov_coord, c0, c1);    
  }
   ...
   *  @overload
   *  @brief Compute intersection of isosurface and grid edge
   *    using linear interpolation.  (scalar_grid supplies scalar values.)
   *  - (Edge=(iend0,direction).)
   *  - Grid edge specified by lower/left endpoint and edge direction.
   *  - Does not return c0 or c1.
   *  @param scalar_grid Scalar grid data.
   *  @param iend0 Lower/left edge endpoint.
   *  @param edge_direction Axis parallel edge direction. (Integer.)
   *  @param[out] isov_coord[] Array of isosurface vertex coordinates.
  ...
  inline void compute_isosurface_grid_edge_intersection_linear
  (const SGRID_TYPE & scalar_grid, const ISOVALUE_TYPE isovalue,
   const VTYPE iend0, int edge_direction, CTYPEV isov_coord[])
  {
    typedef typename SGRID_TYPE::SCALAR_TYPE SCALAR_TYPE;
    
    // Compute isov_coord[edge_direction].
    const VTYPE iend1 = scalar_grid.NextVertex(iend0, edge_direction);
    const SCALAR_TYPE s0 = scalar_grid.Scalar(iend0);
    const SCALAR_TYPE s1 = scalar_grid.Scalar(iend1);

    compute_isosurface_grid_edge_intersection_linear
      (scalar_grid, isovalue, iend0, edge_direction, s0, s1, isov_coord);
  }
   ...
   *         (Get scalar values at endpoints.)
   *  @details
  ...
  template <typename SGRID_TYPE, typename ISOVALUE_TYPE,
            typename STYPE0, typename STYPE1,
            typename VTYPE, typename CTYPEO, typename CTYPEV>
  inline void compute_isosurface_grid_edge_intersection_linear_offset
  (const SGRID_TYPE & scalar_grid, const ISOVALUE_TYPE isovalue,
   const VTYPE iend0, const int edge_direction,
   const STYPE0 s0, const STYPE1 s1,
   const CTYPEO offset, CTYPEV isov_coord[])
  {
    ...
    compute_isosurface_grid_edge_intersection_linear
      (scalar_grid, isovalue, iend0, edge_direction,
       s0, s1, isov_coord, c0, c1);

    const CTYPEV minc = c0 + offset;
    const CTYPEV maxc = c1 - offset;
   ...
   CTYPE isov_coord[])
  {
    ...
    for (NTYPE i = 0; i < nume; i++) {
      const VTYPE iend0 = isopoly_info[i].GridEdgeEndpoint0();
      const int edge_direction = isopoly_info[i].GridEdgeDirection();

      CTYPE * isov_coord_i = isov_coord + (dimension*i);

      compute_isosurface_grid_edge_intersection_linear
        (scalar_grid, isovalue, iend0, edge_direction, isov_coord_i);
    }
  }
  ...
  void compute_all_isov_coord_on_grid_edge_linear_offset
  (const SGRID_TYPE & scalar_grid, const ISOVALUE_TYPE isovalue,
   const std::vector<ISOPOLY_INFO_TYPE> & isopoly_info,
   const CTYPEO offset, CTYPEV isov_coord[])
  {
