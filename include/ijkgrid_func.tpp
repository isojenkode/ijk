...
namespace IJK {
   ...
   ITYPE2 * cube_vertex_increment)
  {
    ...
    if (axis_increment == NULL) {
      error.AddMessage
        ("Programming error. Array axis_increment[] not allocated.");
      error.AddMessage("Array axis_increment[] must be allocated and set");
      error.AddMessage("  before calling compute_cube_vertex_increment.");
      throw error;
    }

    if (cube_vertex_increment == NULL) {
      error.AddMessage
        ("Programming error. Array cube_vertex_increment[] not allocated");
      error.AddMessage("Array cube_vertex_increment[] must be allocated");
      error.AddMessage("  before calling compute_cube_vertex_increment.");
      throw error;
    }
  ...
  /*!
   *  @brief Compute increment to add to vertex 0 to compute vertex j
   *    of hypercube.
   */
  template <typename CUBEV_BITSET_TYPE, typename ITYPE>
  ITYPE compute_cube_vertex_j_increment
  (const int dimension, const CUBEV_BITSET_TYPE & bitset,
   const ITYPE axis_increment[])
  {
    ITYPE increment_j = 0;
  ...
  /*!
   *  @brief Compute array cube_vertex_bitset[] representing location
   *    of each cube vertex.
   *  @param dimension  Dimension of grid.
   *  @param[out] cube_vertex_bitset[i] Bitset representing location
   *    of cube vertex i.
   ...
   *  @pre Array cube_vertex_bitset[] is allocated with size 
   *    at least number of cube vertices.
   */
  template <typename BITSET_TYPE>
  void compute_cube_vertex_bitset
  (const int dimension, BITSET_TYPE cube_vertex_bitset[])
  {
    const int num_cube_vertices = compute_num_cube_vertices(dimension);
    IJK::PROCEDURE_ERROR error("compute_cube_vertex_bitset");
