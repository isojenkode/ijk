...
namespace IJK {
  ...
  /*!
   *  @brief Class storing scalar grid information.
  ...
  protected:
    ...
    /*!
     *  @brief Array of scalar value of grid vertices.
     *  - Grid vertex iv has scalar value scalar[iv].
     */
    STYPE * scalar;
  ...
  public:
    ...
    /// Set scalar values of boundary vertices to scalar values
    /// at neighboring interior vertices.
    /// @pre Grid has axis size at least 3 in every direction.
    void SetBoundaryFromInteriorNeighbors();
  ...
  public:
    ...
    /// @brief Compute min and max of each region.
    ...
    /*!
     *  @brief Compute min and max of each region.
     *  - Version that includes an offset edge length.
     *  @param offset_edge_length Offset added to each region.
     *    - Scalar values are computed over vertices in range
     *      [minx-offset_edge_length, minx+region_edge_length+offset_edge_length,
     *       miny-offset_edge_length, miny+region_edge_length+offset_edge_length,
     *       ...]
     */
    void ComputeMinMax
    (const DTYPE dimension, const ATYPE * axis_size,
    ...
    /*!
     *  @brief Compute min and max of each region.
     *  - Version where scalar values are stored in scalar_grid.
     */
    template <typename _GRID_CLASS2>
    void ComputeMinMax
    (const SCALAR_GRID_BASE<_GRID_CLASS2,STYPE> & scalar_grid,
     const ATYPE region_edge_length);
  ...
  template <typename GRID_CLASS, typename STYPE>
  void SCALAR_GRID_BASE<GRID_CLASS,STYPE>::
  SetBoundaryFromInteriorNeighbors()
  {
    FACET_VERTEX_LIST<VTYPE> facet_vertex_list(*this, 0, true);

    for (DTYPE d = 0; d < this->Dimension(); d++) {

      if (d > 0) { facet_vertex_list.GetVertices(*this, d); }
      if (this->AxisSize(d) < 2) { continue; }

      const VTYPE axis_increment_d = this->AxisIncrement(d);
      const VTYPE last_vertex_increment_d = 
        (this->AxisSize(d)-1)*axis_increment_d;

      for (NTYPE i = 0; i < facet_vertex_list.NumVertices(); i++) {
        const VTYPE iv0 = facet_vertex_list.VertexIndex(i);
        const VTYPE iv0_adj = iv0 + axis_increment_d;

        this->Set(iv0, this->Scalar(iv0_adj));

        // iv1 is index of vertex on upper/right facet.
        const VTYPE iv1 = iv0 + last_vertex_increment_d;
        const VTYPE iv1_adj = iv1 - axis_increment_d;

        this->Set(iv1, this->Scalar(iv1_adj));
      }
    }
  }
  ...
  */
  ...
  (const GTYPE & scalar_grid2, const PTYPE supersample_period)
  {
    ...
    for (ATYPE x0 = 0; x0 < scalar_grid2.AxisSize(0); x0++) {
      const VTYPE x1 = VTYPE(x0)*supersample_period;
  ...
  (const PTYPE supersample_period)
  {
    ...
    for (DTYPE d = 0; d < this->Dimension(); d++) {
      ...
      for (ATYPE x = 0; x+1 < this->AxisSize(d); x += supersample_period) {

        const VTYPE inc0 = x*axis_increment[d];
        const VTYPE inc1 = inc0 + supersample_period*axis_increment[d];
        for (VTYPE i = 0; i < numv; i++) {
          const VTYPE v0 = vlist[i] + inc0;
          const VTYPE v1 = vlist[i] + inc1;
