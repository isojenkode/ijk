...
/*
  ...
  Copyright (C) 2009-2025 Rephael Wenger
...
namespace IJK {
   ...
   NUM_TYPE & num_non_ambig_changed)
  {
    ...
    IJK::PROCEDURE_ERROR error
      ("recompute_cube_isotable_info_on_grid_ridges");
        ...
        if (num_ambig_facets == ONE) {
          ...
          if (facet_intersection_table.IsSomeFacetOrthogonalTo
            ...
            scalar_grid.PrintIndexAndCoord
              (cerr, "Changing table index for cube ", cube_index, "");
            cerr << " from " << table_index << " to " << table_index2 << endl;
            cerr << "  vertex_ridge_bits: " << vertex_ridge_bits << endl;
            */
   ...
   *  - Use GRID_CUBE_TYPE::list_loc_of_adjacent_cube to get
   *    adjacent cube.
   *  @pre Routine set_all_adjacent_grid_cubes() must be called
   *    on cube_list[] before this routine is called.
   *  @tparam GRID_CUBE_TYPE Should be GRID_CUBE_ISOVERT_X or
   *    be derived from GRID_CUBE_ISOVERT_X.
   *  @param isodual_table Dual isosurface lookup table.
   *    - Should include ambiguity information and functions.
   */
  template <typename GRID_TYPE, typename ISODUAL_TABLE,
            typename GRID_CUBE_TYPE, typename NTYPE>
  void split_non_manifold_isov_pairs_ambig1_using_adjacent
  (const GRID_TYPE & grid,
   const ISODUAL_TABLE & isodual_table,
   std::vector<GRID_CUBE_TYPE> & cube_list, 
   NTYPE & num_split)
  {
    ...
    IJK::PROCEDURE_ERROR error("split_non_manifold_isov_pairs_ambig1_B");
    ...
    for (SIZE_TYPE i0 = 0; i0 < cube_list.size(); i0++) {
      ...
      if (num_isov0 == 1) {
        if (isodual_table.NumAmbiguousFacets(it0) == 1) {
          if (cube_list[i0].is_adjacent_cube_set) {
            const SIZE_TYPE i1 =
              cube_list[i0].list_loc_of_adjacent_cube;
            ...
            // Ambiguous facet is on grid boundary.
  ...
  template <typename GRID_TYPE, typename ISODUAL_TABLE,
            typename GRID_CUBE_TYPE,  typename NTYPE>
  void split_non_manifold_isov_pairs_using_adjacent
  (const GRID_TYPE & grid,
   const ISODUAL_TABLE & isodual_table,
   std::vector<GRID_CUBE_TYPE> & cube_list, 
   NTYPE & num_split)
  {
    ...
    split_non_manifold_isov_pairs_ambig1_using_adjacent
      (grid, isodual_table, cube_list, num_split1);
    ...
    if (dimension > 3) {
      ...
      // *** SHOULD SEE IF THIS CAN BE REFACTORED TO AVOID USING index_to_cube_list[] ***
      split_non_manifold_isov_pairs_ambig2_using_index
  ...
  // **************************************************
  //! @name SPLIT BASED ON index_to_cube_list[].
  // **************************************************

  //@{

  /*!
   *  @base Set index_to_cube_list.
   *  @param cube_list[] List of cubes.
   *  @param[out] index_to_cube_list[icube] Location of cube icube
   *    in array cube_list[].
   */
   ...
   *  - Also splits isosurface vertices in cube with one ambiguous facet
   *    that lies on the grid boundary.
   *  - Use array index_to_cube_list[] to determine split.
   *  @param isodual_table Dual isosurface lookup table.
   *    - Should include ambiguity information and functions.
   *  @param index_to_cube_list[icube] Location of cube icube
   *    in array cube_list[].
   */
  template <typename GRID_TYPE, typename ISODUAL_TABLE,
            typename GRID_CUBE_TYPE, typename INDEX_TYPE,
            typename NTYPE>
  void split_non_manifold_isov_pairs_ambig1_using_index
  (const GRID_TYPE & grid,
   const ISODUAL_TABLE & isodual_table,
   ...
   NTYPE & num_split)
  {
    ...
    IJK::PROCEDURE_ERROR
      error("split_non_manifold_isov_pairs_ambig1_using_index");
  ...
  template <typename GRID_TYPE, typename ISODUAL_TABLE,
            typename GRID_CUBE_TYPE, typename INDEX_TYPE,
            typename NTYPE>
  void split_non_manifold_isov_pairs_ambig2_using_index
  (const GRID_TYPE & grid,
   const ISODUAL_TABLE & isodual_table,
   const INDEX_TYPE index_to_cube_list[],
   std::vector<GRID_CUBE_TYPE> & cube_list, 
   NTYPE & num_split)
  {
    ...
    IJK::PROCEDURE_ERROR
      error("split_non_manifold_isov_pairs_ambig2_using_index");
  ...
  template <typename GRID_TYPE, typename ISODUAL_TABLE,
            typename GRID_CUBE_TYPE,  typename NTYPE>
  void split_non_manifold_isov_pairs_using_index
  (const GRID_TYPE & grid,
   const ISODUAL_TABLE & isodual_table,
   std::vector<GRID_CUBE_TYPE> & cube_list, 
   NTYPE & num_split)
  {
    ...
    split_non_manifold_isov_pairs_ambig1_using_index
      (grid, isodual_table, index_to_cube_list.PtrConst(),
  ...
  template <typename CI_TYPE, typename TI_TYPE, 
            typename NTYPE, typename _ISOV_TYPE>
  ...
  template <const int GRID_DIMENSION, typename CI_TYPE, typename TI_TYPE, 
            typename NTYPE, typename ISOV_TYPE, typename COORD_TYPE>
  class GRID_CUBE_ISOVERT_AND_CUBE_COORD:public
  GRID_CUBE_ISOVERT<CI_TYPE,TI_TYPE,NTYPE,ISOV_TYPE> {
  ...
  <GRID_DIMENSION, CI_TYPE, TI_TYPE, NTYPE, ISOV_TYPE, COORD_TYPE>::
  ...
  /*!
   *  @brief Extended information about dual contouring isosurface vertices
   *    in a grid cube.
   *  - Contains index of some adjacent cube that shares
   *    an ambiguous active facet with this->cube_index,
   *    if such a cube exist.
   *  - Otherwise, contains index of any adjacent cube that shares
   *    an active facet with this->cube_index.
   */
  template <typename CI_TYPE, typename TI_TYPE, 
            typename NTYPE, typename FTYPE>
  class GRID_CUBE_ISOVERT_X:
    public GRID_CUBE_ISOVERT<CI_TYPE, TI_TYPE, NTYPE, FTYPE> {

  public:

    /*!
     *  - Location in list of some adjacent cube that shares
     *    an ambiguous facet with this->cube_index,
     *    if such a cube exist.
     */
    CI_TYPE list_loc_of_adjacent_cube;

    /// @brief True if this->cube_index and adjacent cube
    ///   share an ambiguous facet.
    bool is_adjacent_cube_set;

    GRID_CUBE_ISOVERT_X()
    { is_adjacent_cube_set = false; }
  };

  
  template <typename T>
  concept HasListLocOfAdjacentCube = requires (T t) {
    { t.list_loc_of_adjacent_cube };
  };

  // ***************************************************************
  //! @name COMPUTE GRID_CUBE_ISOVERT INFORMATION
  // ***************************************************************

  /*!
   *  @brief Set list_loc_of_adjacent_cube
   *    in active_cube_list[cube0_list_loc].
   */
  template <typename ISODUAL_TABLE, typename LTYPE,
            typename GRID_CUBE_TYPE>
  void set_adjacent_grid_cube
  (const ISODUAL_TABLE & isodual_table,
   const LTYPE cube0_list_loc, const LTYPE cube1_list_loc,
   ...
   std::vector<GRID_CUBE_TYPE> & active_cube_list)
  {
    ...
    const TABLE_INDEX table_index0 =
      active_cube_list[cube0_list_loc].table_index;

    if (active_cube_list[cube0_list_loc].is_adjacent_cube_set)
      { return; }
    
    if (isodual_table.IsFacetAmbiguous(table_index0, ishared_facet0)) {
      active_cube_list[cube0_list_loc].list_loc_of_adjacent_cube =
        cube1_list_loc;
      active_cube_list[cube0_list_loc].is_adjacent_cube_set = true;
    }
  }


  /// @brief Set list_loc_of_adjacent_cube in all cubes in active_cube_list[].
  template <typename GRID_TYPE, typename ISODUAL_TABLE,
            typename LTYPE, typename NTYPEP, typename GRID_CUBE_TYPE>
  void set_all_adjacent_grid_cubes
  (const GRID_TYPE & grid, const ISODUAL_TABLE & isodual_table,
   const LTYPE isopoly_cube_loc[],
   const NTYPEP num_isopoly,
   std::vector<GRID_CUBE_TYPE> & active_cube_list)
  {
    typedef typename GRID_CUBE_TYPE::CUBE_INDEX_TYPE CUBE_INDEX_TYPE;
    typedef typename ISODUAL_TABLE::TABLE_INDEX TABLE_INDEX;

    const int dimension = isodual_table.Dimension();
    
    const CUBE_FACE_INFO<int,int,int> cube_info(dimension-1);
    const int num_cube_vertices = cube_info.NumVertices();
    const int num_cube_edges = cube_info.NumEdges();
    std::vector<int> endpoint0(num_cube_edges);
    std::vector<int> endpoint1(num_cube_edges);

    // Set endpoint0[] and endpoint1[].
    for (int ie = 0; ie < num_cube_edges; ie++) {
      endpoint0[ie] = cube_info.EdgeEndpoint(ie,0);
      endpoint1[ie] = cube_info.EdgeEndpoint(ie,1);
    }
    
    for (NTYPEP ipoly = 0; ipoly < num_isopoly; ipoly++) {
      const LTYPE * isopoly_cube_loc_i =
        isopoly_cube_loc + ipoly*num_cube_vertices;

      for (int ie = 0; ie < cube_info.NumEdges(); ie++) {
        const int iend0 = endpoint0[ie];
        const int iend1 = endpoint1[ie];
        const LTYPE j0 = *(isopoly_cube_loc_i+iend0);
        const LTYPE j1 = *(isopoly_cube_loc_i+iend1);

        const TABLE_INDEX table_index0 = active_cube_list[j0].table_index;
        if (isodual_table.NumAmbiguousFacets(table_index0) < 1)
          { continue; }
        const TABLE_INDEX table_index1 = active_cube_list[j1].table_index;
        if (isodual_table.NumAmbiguousFacets(table_index1) < 1)
          { continue; }

        if (active_cube_list[j0].is_adjacent_cube_set &&
            active_cube_list[j1].is_adjacent_cube_set)
          { continue; }
        
        const CUBE_INDEX_TYPE icube0 = active_cube_list[j0].cube_index;
        const CUBE_INDEX_TYPE icube1 = active_cube_list[j1].cube_index;
        const int ishared_facet0 = grid.SharedFacet(icube0, icube1);
        const int ishared_facet1 =
          (ishared_facet0 + dimension)%(2*dimension);
        
        set_adjacent_grid_cube
          (isodual_table, j0, j1, ishared_facet0, active_cube_list);
        set_adjacent_grid_cube
          (isodual_table, j1, j0, ishared_facet1, active_cube_list);
      }
    }
  }
   ...
   *  @brief Set list_loc_of_adjacent_cube in all cubes in active_cube_list[]
   *    (C++ vector).
   *  - Version using C++ STL vector active_cube_list[].
   */
  template <typename GRID_TYPE, typename ISODUAL_TABLE, typename LTYPE,
            typename GRID_CUBE_TYPE>
  void set_all_adjacent_grid_cubes
  (const GRID_TYPE & grid,
   const ISODUAL_TABLE & isodual_table,
   const std::vector<LTYPE> & isopoly_cube_loc,
   std::vector<GRID_CUBE_TYPE> & active_cube_list)
  {
    typedef typename std::vector<GRID_CUBE_TYPE>::size_type SIZE_TYPE;

    const int num_vertices_per_isopoly = grid.NumFacetVertices();
    const SIZE_TYPE num_isopoly =
      isopoly_cube_loc.size()/num_vertices_per_isopoly;

    set_all_adjacent_grid_cubes
      (grid, isodual_table, IJK::vector2pointer(isopoly_cube_loc),
       num_isopoly, active_cube_list);
  }
  ...
  template <typename GRID_TYPE, typename ISODUAL_TABLE, typename SCALAR_TYPE,
            ...
            typename GRID_CUBE_TYPE, 
            typename ISOV_TYPE, typename ISOV_INDEX_TYPE,  typename NTYPE>
  requires (HasListLocOfAdjacentCube<GRID_CUBE_TYPE>)
  void split_dual_isovert
  (const GRID_TYPE & scalar_grid, const ISODUAL_TABLE & isodual_table,
   const SCALAR_TYPE isovalue,
   const std::vector<CINDEX_TYPE> & isopoly_cube_loc, 
