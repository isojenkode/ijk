...
/*!
 *  @brief Construct triangulated dual isosurface, one isosurfae vertex
 *     per cube.
 */
void construct_dual_triangulated_isosurface_single_isov
 ...
 DUALISO_TIME & dualiso_time, IO_TIME & io_time)
{
  ...
  const int num_facet_vertices =
    dualiso_data.ScalarGrid().NumFacetVertices();
  ...
  DUAL_TRIANGULATED_ISOSURFACE dual_isosurface
    (dimension, num_facet_vertices);
  std::vector<ISOPOLY_INFO_TYPE> isopoly_info;
  ...
  if (output_info.flag_report_all_isov) {
    write_isov_info
      (output_info, dualiso_data.ScalarGrid(), active_cube_list,
...
/*!
 *  @brief Construct triangulated dual isosurface, allow multiple
 *     isosurface vertices per cube.
 */
void construct_dual_triangulated_isosurface_multi_isov
(const DUALISO_DATA & dualiso_data, const SCALAR_TYPE isovalue, 
 OUTPUT_INFO & output_info,
 DUALISO_TIME & dualiso_time, IO_TIME & io_time)
{
  ...
  if (output_info.flag_report_all_isov) {
    write_multi_isov_info
      (output_info, dualiso_data.ScalarGrid(), isodual_table,
       isov_list, dual_isosurface.vertex_coord);
  }
}


/*!
 *  @brief Construct dual cube complex isosurface, one isosurface
 *    vertex per cube.
 *  - Output mesh elements are cubes (line segments, quads, hexahedra, ...)
 */
void construct_dual_cube_complex_isosurface_single_isov
(const DUALISO_DATA & dualiso_data, const SCALAR_TYPE isovalue, 
 OUTPUT_INFO & output_info,
 DUALISO_TIME & dualiso_time, IO_TIME & io_time)
{
...
/*!
 *  @brief Construct dual cube complex isosurface, allow multiple
 *    isosurface vertices per cube.
 *  - Output mesh elements are cubes (line segments, quads, hexahedra, ...)
 */
void construct_dual_cube_complex_isosurface_multi_isov
(const DUALISO_DATA & dualiso_data, const SCALAR_TYPE isovalue, 
 OUTPUT_INFO & output_info,
 DUALISO_TIME & dualiso_time, IO_TIME & io_time)
{
  ...
  for (unsigned int i = 0; i < io_info.isovalue.size(); i++) {
    ...
    if (dimension == DIM3 && 
      ...
      if (allow_multiple_isov) {
        construct_dual_triangulated_isosurface_multi_isov
          (dualiso_data, isovalue, output_info,
           dualiso_time, io_time);
      }
      else {
        construct_dual_triangulated_isosurface_single_isov
          (dualiso_data, isovalue, output_info,
           dualiso_time, io_time);        
      }
    }
      ...
      if (allow_multiple_isov) {
        construct_dual_cube_complex_isosurface_multi_isov
          (dualiso_data, isovalue, output_info,
           dualiso_time, io_time);
      }
      else {
        construct_dual_cube_complex_isosurface_single_isov
          (dualiso_data, isovalue, output_info,
           dualiso_time, io_time);        
      }
    }
