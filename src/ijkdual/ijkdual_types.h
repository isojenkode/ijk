...
namespace IJKDUAL {
  ...
  // *** SHOULD BE unsigned char. NO REASON TO BE int. ***
   ...
   */
  typedef enum
    { SIMPLICIAL_COMPLEX, CUBE_COMPLEX, MIXED_MESH, UNDEFINED_MESH_TYPE }
    MESH_TYPE;
  ...
  typedef enum
    { CUBE_CENTER, CENTROID_EDGE_ISO, DIAGONAL_INTERPOLATION,
      ...
      UNDEFINED_VERTEX_POSITION_METHOD } 
    VERTEX_POSITION_METHOD;


  /*!
   *  @brief Isosurface isosurface-(grid edge) intersection offset method.
   *  - NO_OFFSET: Do not offset.
   *  - OFFSET_ONLY_MULTI_IN_CUBE: Only apply offset in cubes that contain
   *    multiple isosurface vertices.
   *  - OFFSET_ALL: Offset all isosurface-(grid edge) intersections.
   */
  typedef enum
    { NO_OFFSET, OFFSET_ONLY_MULTI_IN_CUBE, OFFSET_ALL,
  ...
  /*!
   *  @brief Methods of separating multiple isosurface vertices
   *    in single cube.
   *  - SEPARATE_BY_CUBE_CENTER: Separate by planes through
   *    the cube center.
   *  - SEPARATE_BY_PLANES: Separate by planes orthogonal
   *    to the grid axes.
   *  - SEPARATE_USING_CENTROID_POS: Apply centroid positioning
   *    for multiple isosurface vertices in a single cube.
   *    Also apply centroid positioning to doubly connected
   *    isosurface vertices.
   */
  typedef enum { SEPARATE_BY_CUBE_CENTER, SEPARATE_BY_PLANES,
                 UNDEFINED_ISOV_SEPARATION_METHOD }
    ISOV_SEPARATION_METHOD;

  /*!
   *  @brief Methods of repositioning doubly connected vertex.
   *  - An isosurface vertex is doubly connected to cube c if it is adjacent
   *    to two (or more) isosurface vertices in cube c.
   *  - Currently just applied to 3D.
   *  - DC_POS_CLAMPIII_ONE_THIRD:
   *    - If adjacent cube has 2 vertices, clamp between 2 vertices.
   *    - If adjacent cube has 3 or more vertices,
   *      clamp to [1/3,2/3] of cube.
   *  - DC_POS_CLAMP_ALL: Always clamp doubly connected vertex between
   *    2 vertices in adjacent cube.
   *  - DC_POS_CENTERIII:
   *    - If adjacent cube has 2 vertices, clamp between 2 vertices.
   *    - If adjacent cube has 3 or more vertices, align with cube center.
   *  - DC_POS_CENTER_ALL: Always align doubly connected vertex
   *    with cube center.
   */
  typedef enum { DC_POS_CLAMPIII_ONE_THIRD,
                 DC_POS_CLAMP_ALL,
                 DC_POS_CENTERIII, DC_POS_CENTER_ALL,
  ...
  /*!
   *  @anchor randomPosIsovSeparationMethodAnchor
   *  @brief Separation of randomly generated isosurface vertex
   *    positions in cubes containing more than one isosurface vertex.
   *  - RANDOM_POS_NO_SEPARATION: Generate positions without
   *    any separation, (i.e. vertices can be anywhere in the cube.)
   *  - RANDOM_POS_SEPARATE_USING_CENTROID_POS:
   *    - Apply centroid positioning on multiple isosurface vertices
   *      that are in the same cube.
   *    - Also apply to doubly connected isosurface vertices.
   *  - RANDOM_POS_SEPARATE_BY_CUBE_CENTER:
   *    Separate vertices by cube center.
   *  - RANDOM_POS_SEPARATE_BASED_ON_DEGREE:
   *    Separate based on vertex degrees.
