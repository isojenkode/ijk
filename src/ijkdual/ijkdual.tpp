...
namespace IJKDUAL {
  ...
  void position_all_isovertices_single_isov
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    dual_contouring_single_isov
      (dualiso_data.ScalarGrid(), isovalue,
       ...
       active_cube_list, dual_isosurface.isopoly_vert, isopoly_info,
   ...
   *    - Includes scalar grid and algorithm parameters.
   *  @param isovalue Isovalue.
   *  @param isodual_table Dual contouring isosurface lookup table.
   *    - Determines connectivity between isosurface vertices
   *      in cases where cube contains multiple isosurface vertices.
   *  @param dual_isosurface Mesh storing the dual isosurface.
   *    - Includes array of isosurface vertex coordinates 
   *      and array of isosurface polytope vertices.
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    dual_contouring_multi_isov
      (dualiso_data.ScalarGrid(), isovalue, isodual_table,
       dualiso_data, isov_list, dual_isosurface.isopoly_vert,
       isopoly_info, dual_isosurface.vertex_coord, dualiso_info);
   ...
   *  @overload
   *  @brief Dual Contouring Algorithm - grid cubes may contain
   *    multiple isosurface vertices. (Construct isodual tabl.e)
   *  - Version that construcs the dual isosurface lookup table.
   *  @param dualiso_data Data for constructing dual isosurface.
   *    - Includes scalar grid and algorithm parameters.
   *  @param isovalue Isovalue.
   *  @param dual_isosurface Mesh storing the dual isosurface.
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    dual_contouring_multi_isov
      (dualiso_data, isovalue, isodual_table, dual_isosurface,
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    const bool flag_always_split_isov_using_cube_list_index =
      param.flag_always_split_isov_using_cube_list_index;
    ...
    if (flag_split_non_manifold || flag_select_split ||
      ...
      IJK::set_all_adjacent_grid_cubes
        (scalar_grid, isodual_table, isopoly_cube_loc,
         active_cube_list);
      ...
      if (flag_split_non_manifold || flag_split_non_manifold_ambig) {
        if (flag_always_split_isov_using_cube_list_index) {
          IJK::split_non_manifold_isov_pairs_using_index
            (scalar_grid, isodual_table, active_cube_list,
             num_non_manifold_split);
        }
        else {
          IJK::split_non_manifold_isov_pairs_using_adjacent
            (scalar_grid, isodual_table, active_cube_list,
             num_non_manifold_split);          
        }
      }
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    create_isosurface_vertices_multi_isov
      (scalar_grid, isovalue, isodual_table,
       ...
       active_cube_list, isopoly_vert, isov_list,
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    create_isosurface_vertices_multi_isov
      (scalar_grid, isovalue, isodual_table,
       ...
       param, active_cube_list, isopoly_vert, isov_list,
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    construct_multi_isov_mesh_I
      (scalar_grid, isovalue, isodual_table, param, active_cube_list,
       isopoly_vert, isopoly_info, isov_list, merge_data, dualiso_info);

    const std::clock_t t1 = clock();

    position_all_isovertices_multi_isov
      (scalar_grid, isodual_table, isovalue,
       active_cube_list, isov_list, isopoly_vert, isopoly_info,
       param.iso_vertex_position_param, vertex_coord, dualiso_info);

    const std::clock_t t2 = clock();

    if (param.flag_count_ambiguous) {
      // Compute information on ambiguous cubes/facets.
      GRID_SIZE_TYPE num_cubes_with_ambig_facets;
      GRID_SIZE_TYPE num_ambig_grid_facets;
      count_num_ambig
        (scalar_grid, isodual_table, active_cube_list,
         num_cubes_with_ambig_facets, num_ambig_grid_facets);

      dualiso_info.scalar.num_cubes_with_ambig_facets.Set
        (num_cubes_with_ambig_facets);
      dualiso_info.scalar.num_ambig_grid_facets.Set
   ...
   *  - Version that creates merge_data.
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    dual_contouring_multi_isov
      (scalar_grid, isovalue, isodual_table, param,
       active_cube_list, isopoly_vert, isopoly_info,
       isov_list, vertex_coord, merge_data, dualiso_info);
  }
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    dual_contouring_multi_isov
      (scalar_grid, isovalue, isodual_table, manifold_param,
       active_cube_list, isopoly_vert, isopoly_info, isov_list,
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    position_all_isovertices_single_isov
   ...
   */
    ...
    DUALISO_INFO & dualiso_info)
   {
     ...
     const ISO_GRID_EDGE_INTERSECTION_OFFSET_METHOD
       iso_grid_edge_intersection_method =
       iso_vertex_position_param.iso_grid_edge_intersection_offset_method;
     const bool flag_move_all_isov_away_from_cube_boundaries =
       iso_vertex_position_param.flag_move_all_isov_away_from_cube_boundaries;
     const COORD_TYPE iso_grid_edgeI_offset =
       iso_vertex_position_param.iso_grid_edgeI_offset;
     ...
     IJK::PROCEDURE_ERROR error("position_all_isovertices_single_isov");

     std::clock_t t0, t1;

     if (vertex_position_method == CUBE_CENTER) {
       position_all_isov_cube_center
 ...
 # endif
     ...
     else if (iso_grid_edge_intersection_method == OFFSET_ALL) {
       /* OBSOLETE
       position_all_isov_centroid_offset_simple
         (scalar_grid, isovalue, active_cube_list, position_offset,
          vertex_coord);
       */
       position_all_isov_centroid_offset_simple
         (scalar_grid, isovalue, active_cube_list, iso_grid_edgeI_offset,
          vertex_coord);       
     }
     else {
       // default
       position_all_isov_centroid_simple
     ...
     if (flag_move_all_isov_away_from_cube_boundaries) {
       reposition_all_isovert_away_from_cube_facets
       ...
       IJK::clock2seconds
         (t2-t1, dualiso_info.time.position.move_isov_away_from_grid_cube_boundaries);
     }
   ...
   *  @overload
   *  @brief Position dual isosurface vertices. 
   *  - Version using isopoly_vert[] and isopoly_info[].
   *  @pre Each grid cube contains at most one isosurface vertex.
   */
  template <typename GRID_TYPE, typename STYPE,
   ...
   DUALISO_INFO & dualiso_info)
  {
    const VERTEX_POSITION_METHOD vertex_position_method =
      iso_vertex_position_param.vertex_position_method;
    const bool flag_position_method_simple =
      iso_vertex_position_param.flag_position_method_simple;
    const ISO_GRID_EDGE_INTERSECTION_OFFSET_METHOD
      iso_grid_edge_intersection_method =
      iso_vertex_position_param.iso_grid_edge_intersection_offset_method;
    const COORD_TYPE iso_grid_edgeI_offset =
...
# endif
    ...
    else if (flag_position_method_simple) {
      if (iso_grid_edge_intersection_method == OFFSET_ALL) {
        /* OBSOLETE
      ...
      position_all_isov_centroid_offset_single
        (scalar_grid, isovalue, active_cube_list,
         isopoly_vert, isopoly_info, position_offset, vertex_coord);
      */
      position_all_isov_centroid_offset_single
        (scalar_grid, isovalue, active_cube_list,
         isopoly_vert, isopoly_info, iso_grid_edgeI_offset, vertex_coord);      
    }
    else {
      // default
   ...
   DUALISO_INFO & dualiso_info)
  {
    ...
    const ISO_GRID_EDGE_INTERSECTION_OFFSET_METHOD
      iso_grid_edge_intersection_method =
      iso_vertex_position_param.iso_grid_edge_intersection_offset_method;
    const bool flag_reposition_doubly_connected_isosurface_vertices =
      iso_vertex_position_param.flag_reposition_doubly_connected_isosurface_vertices;
    ...
    const bool flag_separate_isov_near_shared_grid_cube_facets =
      iso_vertex_position_param.flag_separate_isov_near_shared_grid_cube_facets;
    const bool flag_separate_isov_near_shared_grid_cube_ridges =
      iso_vertex_position_param.flag_separate_isov_near_shared_grid_cube_ridges;
    const bool flag_separate_isov_near_shared_grid_vertices =
      iso_vertex_position_param.flag_separate_isov_near_shared_grid_vertices;
    const bool flag_separate_iso_poly_near_shared_grid_vertices =
      iso_vertex_position_param.flag_separate_iso_poly_near_shared_grid_vertices;
    const ISOV_SEPARATION_METHOD isov_separation_method =
      iso_vertex_position_param.isov_separation_method;
    const bool flag_separate_multi_isov_in_single_cube =
      iso_vertex_position_param.flag_separate_multi_isov_in_single_cube;
    const bool flag_separate_iso_edges =
      iso_vertex_position_param.flag_separate_iso_edges;
    ...
    const COORD_TYPE position_offset =
      iso_vertex_position_param.position_offset;
    IJK::PROCEDURE_ERROR error("position_all_isovertices_multi_isov");
    ...
    if (vertex_position_method == CUBE_CENTER) {
      ...
      DUAL_TABLE_VERTEX_INFO
        isodual_table_vertex_info(isodual_table);
      
      compute_dual_cube_isotable_vertex_connectivity
        (isodual_table, isodual_table_vertex_info);
      compute_dual_isotable_vertex_degrees
        (isodual_table, isodual_table_vertex_info);      

      position_all_isov_multi_random
        (scalar_grid, isodual_table, isodual_table_vertex_info,
         isovalue, active_cube_list, isov_list,
      ...
      position_all_isov_centroid_offset_multi_simple
        (scalar_grid, isodual_table, isovalue, isov_list,
         iso_grid_edge_intersection_method, iso_grid_edgeI_offset,
         vertex_coord);      
    }
    else {
    ...
    else if (flag_separate_isov_near_shared_grid_cube_facets) {
    ...
    if (flag_move_all_isov_away_from_cube_ridges) {
      ...
      dualiso_info.isov.num_isov_moved_away_from_grid_cube_ridges = num_moved;
    }
    else if (flag_separate_isov_near_shared_grid_cube_ridges) {
      int num_moved = 0;
      separate_isov_near_shared_grid_cube_ridges
        (scalar_grid, isov_list, min_distance, position_offset,
         vertex_coord, num_moved);
      dualiso_info.isov.num_isov_moved_away_from_grid_cube_ridges = num_moved;
    }

    if (flag_separate_isov_near_shared_grid_vertices) {
      int num_moved = 0;
      separate_isov_near_shared_grid_vertices
        (scalar_grid, isov_list, min_distance, position_offset,
         vertex_coord, num_moved);
      dualiso_info.isov.num_isov_moved_away_from_grid_vertices = num_moved;
    }    
    ...
    if (flag_separate_iso_poly_near_shared_grid_vertices) {
      const std::clock_t t0 = clock();

      separate_iso_poly_near_shared_grid_vertices
        (scalar_grid, isopoly_vert, isopoly_info, isov_list,
         min_distance, position_offset, vertex_coord,
         dualiso_info.isov.num_isov_moved_to_separate_iso_poly);

      const std::clock_t t1 = clock();
      IJK::clock2seconds
        (t1-t0, dualiso_info.time.position.separate_iso_poly_near_shared_grid_vertices);
    }

    if (flag_separate_multi_isov_in_single_cube ||
        flag_separate_iso_edges) {
      ...
      if (flag_separate_multi_isov_in_single_cube) {
        if (isov_separation_method == SEPARATE_BY_CUBE_CENTER) {
          separate_all_dual_isov_by_cube_centers
            (scalar_grid, isodual_table,
             isodual_table_vertex_info, active_cube_list,
          ...
          separate_all_dual_isov_by_axis_parallel_planes
            (scalar_grid, isodual_table,
             isodual_table_vertex_info, active_cube_list,
             isopoly_vert, isov_list, position_offset,
             vertex_coord, num_moved);
        }
      }

      if (flag_separate_iso_edges) {
        int num_moved;
      ...
      const std::clock_t t1 = clock();
      IJK::clock2seconds
        (t1-t0, dualiso_info.time.position.separate_multi_isov);
    }

    if (flag_reposition_doubly_connected_isosurface_vertices) {
         
      if (dimension == DIM3) {
        const std::clock_t t0 = clock();

        int num_moved;
        if (doubly_connected_isov_reposition_method ==
            DC_POS_CLAMP_ALL) {

          // *** SHOULD NOT RECREATE isodual_table_vertex_info HERE ***
          DUAL_TABLE_VERTEX_INFO
            isodual_table_vertex_info(isodual_table);      
          compute_dual_cube_isotable_vertex_connectivity
            (isodual_table, isodual_table_vertex_info);
          ...
          reposition_all_doubly_connected_3D_clamp_all
            (scalar_grid, isodual_table, isodual_table_vertex_info,
             active_cube_list, position_offset, vertex_coord, num_moved);
        }
        else {
          reposition_all_doubly_connected_isov_cube3D
            (scalar_grid, isodual_table, active_cube_list,
             position_offset, doubly_connected_isov_reposition_method,
             vertex_coord, num_moved);
        }
      
        dualiso_info.isov.num_repositioned_doubly_connected_isov = num_moved;
      
        const std::clock_t t1 = clock();
        IJK::clock2seconds
          (t1-t0, dualiso_info.time.position.reposition_doubly_connected_isosurface_vertices);
      }
    }
