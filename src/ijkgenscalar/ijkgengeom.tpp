...
/*
  ...
  Copyright (C) 2014-2025 Rephael Wenger
...
#ifndef _IJKGENGEOM_TPP_
#define _IJKGENGEOM_TPP_
...
namespace IJKGENGEOM {
  ...
  public:
    ...
    IJK::SET_VALUE<DIM_TYPE> dimension;
    bool flag_tilt;
    bool flag_flange;
    bool flag_wedge;
    ...
    bool FlagSmoothTip() const
    { return(flag_smooth_tip); }
    ...
    bool CheckNumNear
    (const NUM_TYPE num_near, IJK::ERROR & error) const;
  ...
  public:
    ...
    IJK::SET_VALUE<NUMBER_TYPE> geom_info_index; ///< Index into geom_info[].
    IJK::SET_VALUE<NUMBER_TYPE> num_objects;     ///< Number of cubes, tori, etc.
    IJK::SET_VALUE<SEED_TYPE> random_seed;       ///< Random seed.
    IJK::SET_VALUE<SEED_TYPE> randompos_seed;    ///< Random positions seed.
    IJK::SET_VALUE<SEED_TYPE> randomdir_seed;    ///< Random directions seed.

    /// Base for generating random integer x and the computing base^x.
    IJK::SET_VALUE<NUMBER_TYPE> random_base;     
    ...
    /// Set axis size of all axes to size.
    void SetAxisSize(const AXIS_SIZE_TYPE size);

    /// Set axis size from vector asize.
    void SetAxisSize(const std::vector<AXIS_SIZE_TYPE> & asize);
    ...
    const NUMBER_TYPE RandomBase() const
    { return(random_base.Value()); }
...
#define _GET_COORDINATE_STRING_(_function_name,_obj)                 \
  ...
  {                                                                  \
    IJK::array2string                                                \
      (&(this->_obj[i*Dimension()]), Dimension(), " ", s);           \
  }
...
#define _GET_SCALAR_STRING_(_function_name,_obj)                     \
  ...
  (const NUM_TYPE num_near, IJK::ERROR & error) const
  {
    if (num_near < 1) { return(true); }

    if (dist2near0.size() == 0) {
      error.AddMessage("Programming error.  Missing distance to near planes.");
      error.AddMessage("  No distance to near planes are defined.");
      return(false);
    }

    if (num_near > dist2near0.size()) {
      error.AddMessage("Programming error.  Missing distances to near planes.");
      error.AddMessage("  Only ", dist2near0.size(), " distances are defined.");
      error.AddMessage("  Expected ", num_near, " distances.");
      return(false);
    }
  ...
  _GEOM_PARAM_T_HEADER_
  SetAxisSize(const std::vector<AXIS_SIZE_TYPE> & asize)
  {
    IJK::PROCEDURE_ERROR error("GEOM_PARAM_T.SetAxisSIze");
    ...
    if (asize.size() != Dimension()) {
      error.AddMessage
        ("Programming error. Input axis size does not match dimension.");
      throw error;
    }

    for (DIMENSION_TYPE d = 0; d < Dimension(); d++) 
      { axis_size.push_back(asize[d]); }
  }
