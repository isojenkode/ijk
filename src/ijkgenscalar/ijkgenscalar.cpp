...
NRRD_ENCODING_TYPE nrrd_encoding_type(NRRD_RAW);
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    if (output_gradients) {
      ...
      write_scalar_grid
        (ofilename, scalar_grid, field_param, field_info, nrrd_encoding_type);
      ...
      const bool flag_gzip = (nrrd_encoding_type == NRRD_GZIP);
      write_gradient_grid
...
{
  if (field_param.center.size() == 0) {
    prompt_coordinates("Enter coordinates of square cylinder center",
                       dimension, field_param.center);
  }

  if (field_param.direction.size() == 0) {
    prompt_coordinates("Enter square cylinder axis direction",
                       dimension, field_param.direction);
  }
  ...
  if (field_param.length_difference.size() == 0) {
    prompt_scalar("Enter difference between square cylinder height and width", 
                  field_param.length_difference);
  }
}

// *** NEW VERSION ***
void prompt_square_cylinder
(const int dimension, const int num_cylinders)
{
  prompt_center(dimension, num_cylinders, "square cylinder");
  ...
  if (field_param.flag_tilt) {
    ...
    // Set side direction to (0,1,0,...)
    field_param.side_direction.clear();
    field_param.side_direction.resize(dimension, 0);
    if (dimension > 1)
      { field_param.side_direction[1] = 1; }
  }
...
void prompt_param(const std::string & field_name)
{
  ...
  if (field_name == "cube") {
    ...
    prompt_square_cylinder(dimension, num_objects);
  }
...
(const int dimension, const COORD1_TYPE dir1[], 
 ...
 COORD3_TYPE new_dir[])
{
  const double radians = std::numbers::pi*angle/180.0;
...
void parse_command_line(const int argc, char **argv)
{
  ...
  while (iarg < argc && argv[iarg][0] == '-') {
    ...
    else if (s == "-ascii") { nrrd_encoding_type = NRRD_ASCII; }
    else if (s == "-gzip") { nrrd_encoding_type = NRRD_GZIP; }
    else if (s == "-list") {
...
void usage_msg()
{
  ...
  cerr << "  [-gzip | -ascii] [-s] [-list] [-version] [-help]" << endl;
}
...
void help_msg()
{
  ...
  cerr << "  -ascii:      Output in ascii format." << endl;
