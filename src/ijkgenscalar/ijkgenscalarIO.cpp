 ...
 const std::vector<FIELD_INFO> & field_info)
{
  ...
  if (field_name == "frustrum" || field_name == "cannon") {
  ...
  if (field_name == "cannon") {
    if (field_param.dist2ball_center.size() == 1) {
      field_param.GetDist2BallCenterString(0, s);
      nrrd_header.AddKeyValue("dist2ballCenter", s);
    }
  }
  ...
  if (field_name == "cone") {
    if (field_param.FlagSmoothTip()) {
      nrrd_header.AddKeyValue("smoothTip", "true");
    }
  }
  ...
  if (field_info[field_index].flag_random_seed) {
    string seed_string;
    IJK::val2string(field_param.RandomSeed(), seed_string);
    nrrdKeyValueAdd
  ...
  if (field_info[field_index].flag_maxval) {
    ...
    field_param.GetMaxValString(maxval_string);
    nrrdKeyValueAdd
      (nrrd_header.DataPtr(), "maxValue", maxval_string.c_str());
  }

  if (field_info[field_index].flag_max_exponent) {
    string random_base_string;
    field_param.GetRandomBaseString(random_base_string);
    nrrdKeyValueAdd
      (nrrd_header.DataPtr(), "randomBase", 
       random_base_string.c_str());

    string max_exponent_string;
    field_param.GetMaxExponentString(max_exponent_string);
    nrrdKeyValueAdd
      (nrrd_header.DataPtr(), "maxExponent", 
       max_exponent_string.c_str());
  }

  if (field_name == "isotable_entry") {
    IJK::val2string(field_param.IsotableIndex(), s);
    nrrd_header.AddKeyValue("isotableIndex", s);
  }

  if (field_name == "set" || field_name == "random_int") {
    for (int i = 0; i < field_param.set_scalar_list.size(); i++) {
      if (field_param.set_scalar_list[i].IsTypeSetIsotableIndex()) {
        string key;
        make_string("isotableIndexCube", i, key);
        field_param.GetSetCubeIsotableIndexString(i, s);
        nrrd_header.AddKeyValue(key, s);
      }
      else if (field_param.set_scalar_list[i].IsTypeSetPosVertex()) {
        string key;
        make_string("setPosVertex", i, key);
        field_param.GetSetScalarPosVertexString(i, s);
        nrrd_header.AddKeyValue(key, s);
      }
      else if (field_param.set_scalar_list[i].IsTypeSetPosEdge()) {
        string key;
        make_string("setPosEdge", i, key);
        field_param.GetSetScalarPosEdgeString(i, s);
        nrrd_header.AddKeyValue(key, s);
      }
      else if (field_param.set_scalar_list[i].IsTypeSetPosFacet()) {
        string key;
        make_string("setPosFacet", i, key);
        field_param.GetSetScalarPosFacetString(i, s);
        nrrd_header.AddKeyValue(key, s);
      }
      else if (field_param.set_scalar_list[i].IsTypeSetPosFacetDiagonal()) {
        string key;
        make_string("setPosFacetDiagonal", i, key);
        field_param.GetSetScalarPosFacetDiagonalString(i, s);
        nrrd_header.AddKeyValue(key, s);
      }
    }
  }
 ...
 const NRRD_ENCODING_TYPE nrrd_encoding_type)
{
  ...
  case NRRD_ASCII:
    write_scalar_grid_nrrd_ascii(output_filename, grid, nrrd_header);
    break;
