...
bool flag_report_max_dist(false);
...
// @param[out] max_dist_squared Maximum distance squared found between two coordinates.
template <typename CTYPE_MAX, typename CTYPE_MAX_DIST>
bool diff_coord(const int dimension,
                ...
                CTYPE_MAX & max_diff,
                CTYPE_MAX_DIST & max_dist_squared)
{
  ...
  if (flag_found_diff) {
    CTYPE_MAX_DIST dist_squared;
    const COORD_TYPE * coord0 =
      IJK::vector2pointer(vertex_coord0) + i*dimension;
    const COORD_TYPE * coord1 =
      IJK::vector2pointer(vertex_coord1) + i*dimension;    
    IJK::compute_distance_squared
      (dimension, coord0, coord1, dist_squared);
    max_dist_squared = std::max(max_dist_squared, dist_squared);
  }
 ...
 const COORD_TYPE min_diff, bool & flag_diff, int & num_diff)
{
    ...
    for (int i = 0; i < numv0; i++) {
      ...
      if (diff_coord
          (dimension, vertex_coord0, vertex_coord1, i,
           min_diff, max_diff, max_dist_squared)) {
        flag_diff = true;
  ...
  if (flag_diff) {
    ...
    if (num_diff > 0) {
      ...
      if (flag_report_max_dist) {
        cout << "  (Max distance: "
             << std::sqrt(max_dist_squared) << ")"
             << endl;
      }
    }
  }
...
void parse_command_line(int argc, char **argv)
{
  ...
  while (iarg < argc && argv[iarg][0] == '-') {
    ...
    if (s == "-help") { help_msg(); }
      ...
      min_diff = get_arg_float(iarg, argc, argv);
      iarg++;
    }
    else if (s == "-sort") {
      flag_sort = true;
      ...
      flag_report_max_dist = true;
    }
    else if (s == "-terse") {
      flag_terse = true;
    }
    else if (s == "-n") {
      max_num_poly_out = get_arg_int(iarg, argc, argv);
      max_num_coord_out = max_num_poly_out;
      iarg++;
    }
    else { usage_error(); }
...
void usage_msg()
{
  ...
  cerr << "OPTIONS:" << endl;
  cerr << "  [-min_diff {D}] [-sort] [-n {N}] [-max_dist]" << endl;
...
void help_msg()
{
  ...
  cerr << "  -min_diff {D}: Coordinates which differ by {D} are different."
       << endl;
  cerr << "  -n {N}:        Print at most {N} different coordinates" << endl
       << "                 or {N} different polytopes." << endl;
  cerr << "  -sort:         Sort polytope vertices in increasing order."
       << endl;
  cerr << "                 Ignores any polytope structure and/or orientation."
       << endl;
  cerr << "  -max_dist:     Report maximum distance between matching vertices"
       << endl
       << "                 in {file1} and {file2}."
       << endl;
