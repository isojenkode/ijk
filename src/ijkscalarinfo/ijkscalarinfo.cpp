/*!
 *  @file ijkscalarinfo.cpp
 *  @brief Get info from scalar grid data.
 *  - Version 0.6.0
 */
...
bool flag_print_grid = false;
bool flag_grid_lines = false;
...
void output_grid
(ostream & output, const bool flag_grid_lines, const SGRID & scalar_grid);
...
typedef enum
  {FREQ_PARAM, EDGE_PARAM, CUBE_PARAM, GRADIENT_PARAM, ZERO_GRADIENT_PARAM,
   ...
   PRINT_GRID_PARAM, PRINT_GRIDB_PARAM,
...
const char * parameter_string[] = 
  {"-freq", "-edge", "-cube", "-gradient", "-zero_gradient", 
   ...
   "-vertex", "-vc", "-cc", "-print_grid", "-print_gridB",
   "-subsample", "-o", "-file_label", 
...
void check_input(const SGRID & scalar_grid);
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    grid_nrrd_in.ReadScalarGrid(input_filename, scalar_grid, error);
    ...
    check_input(scalar_grid);

    if (!flag_subsample) {

      if (flag_print_grid) 
        { output_grid(cout, flag_grid_lines, scalar_grid);  }
      ...
      if (flag_print_grid) 
        { output_grid(cout, flag_grid_lines, subsampled_grid); }
 ...
 DATA_TABLE & table)
{
  ...
  compute_increment(bucket_grid, axis_increment);
 ...
 DATA_TABLE & table)
{
  ...
  for (VERTEX_INDEX k = 0; k < num_outer_vertices; k++) {
    ...
    bucket_grid.ComputeCoord(iv0, coord0);
 ...
 DATA_TABLE & table)
{
      ...
      for (NUM_TYPE j = 0; j < num_vertices_in_grid_facet_interior; j++) {
        ...
        for (AXIS_SIZE_TYPE i = boundary_width; 
          ...
          GRADIENT_TYPE L = LaplaceInterior2
            (dimension, axis_increment, iv, scalar);
  ...
  compute_increment(dimension, axis_size, extended_increment);
  ...
  for (NUM_TYPE j = 0; j < num_boundary_vertices; j++) {
    ...
    GRADIENT_TYPE L = LaplaceBnd(dimension, extended_increment, iv, scalar);
    NUM_TYPE irow = bucket_grid.Scalar(iv);
 ...
 DATA_TABLE & table)
{
  ...
  construct_contour_tree<CUBE_VERTEX_BITSET_SIZE>
...
void output_grid_info(ostream & out, const SGRID & scalar_grid)
{
  if (flag_subsample) {
    out << "Subsampled grid." << endl;
    out << "Subsample resolution: " << subsample_resolution << endl;
    out << endl;
  }
  out << "Volume dimension: " << scalar_grid.Dimension() << endl;
  if (!flag_subsample) {
    out << "# vertices per grid axis:";
  }
  else {
    out << "# vertices per subsampled grid axis:";
  }
  for (NUM_TYPE d = 0; d < scalar_grid.Dimension(); d++) {
    out << "  " << scalar_grid.AxisSize(d);
  }
  out << endl;
  ...
  NUM_TYPE num_cubes = scalar_grid.ComputeNumCubes();
  if (!flag_subsample) {
    out << "# grid vertices:  " << num_vertices << endl;
    out << "# grid edges:  " << num_edges << endl;
    out << "# grid cubes:  " << num_cubes << endl;
  }
  else {
    out << "# subsampled grid vertices:  " << num_vertices << endl;
    out << "# subsampled grid cubes:  " << num_cubes << endl;
  }
  if (!flag_subsample) { out << "Min/max scalar values:  "; }
  else { out << "Min/max subsampled scalar values:  "; }
  out << scalar_grid.FindMinScalar() << "  "
  ...
  if (!flag_subsample) {
    out << "# vertices with scalar value 0:  " << num_zero;
    if (num_vertices > 0) {
      out << " ";
      print_percent(out, num_zero, num_vertices);
    }
    out << endl;
    out << "# cubes with all vertex scalar values 0:  " << num_zero_cubes;
    if (num_cubes > 0) {
      out << " ";
      print_percent(out, num_zero_cubes, num_cubes);
    }
    out << endl;
  }
  else {
    out << "# subsampled vertices with scalar value 0:  " 
           << num_zero << endl;
    out << "# subsampled cubes with all vertex scalar values 0:  " 
           << num_zero_cubes << endl;
  }
  ...
  if (flag_uniform) {
    if (num_vertices > 0) {
      if (!flag_subsample) {
        out << "All vertices have scalar value:  " 
               << scalar_grid.Scalar(0) << endl;
      }
      else {
        out << "All subsampled vertices have scalar value:  " 
               << scalar_grid.Scalar(0) << endl;
      }
    }
  }
    ...
    if (!flag_subsample) {
      out << "Min interval between distinct scalar values:  "
             << min_scalar_interval << endl;
    }
    else {
      out << "Min interval between distinct subsampled scalar values:  "
             << min_scalar_interval << endl;
    }
  }
                        ...
                        const int vertex_index)
{
  ...
  if (vertex_index < scalar_grid.NumVertices()) {
    ...
    output << "Scalar value: " << scalar_grid.Scalar(vertex_index)
           << endl;
    scalar_grid.ComputeCoord(vertex_index, coord);
    output << "Coordinates:";
                      ...
                      const std::vector<GRID_COORD_TYPE> & cube_coord)
{
  ...
  for (int d = 0; d < dimension; d++) {
    ...
    if (scalar_grid.AxisSize(d) < TWO) {
      error.AddMessage("Programming error. Illegal cube coordinate.");
      error.AddMessage("  Scalar grid contains no cubes.");
      error.AddMessage
        ("  scalar_grid.AxisSize(", d, "): ", scalar_grid.AxisSize(d));
      throw error;
    }
    
    if (cube_coord[d] < 0 || cube_coord[d]+1 >= scalar_grid.AxisSize(d)) {
      error.AddMessage("Programming error. Illegal cube coordinate.");
      error.AddMessage("  cube_coord[", d, "]: ", cube_coord[d], ".");

      if (cube_coord[d] < 0) {
        error.AddMessage("  Cube coordinate must be non-negative.");
      }
      else {
        error.AddMessage
          ("  scalar_grid.AxisSize(", d, "): ", scalar_grid.AxisSize(d));
        error.AddMessage("  Cube coordinate must be less than ",
                         scalar_grid.AxisSize(d)-1, ".");
      }
      throw error;
    }
  }
...
void output_grid_dimensions(ostream & out, const INFO_GRID & grid)
{
  out << "Grid: ";
  for (int d = 0; d < grid.Dimension(); d++) {
    out << grid.AxisSize(d);
    if (d+1 < grid.Dimension()) { out << "x"; }
  }
  out << endl;
}

void print_scalar(ostream & out, const int field_width, const SCALAR_TYPE x)
{
  int x_int;

  if (x == 0) { out << setw(field_width) << x; }
  else {
    x_int = int(x);
    if (x != x_int) { out << setw(field_width) << x; }
    else {
      int r = x_int%1000;
      if (r == 0) {
        out << (setw(field_width-1)) << x_int/1000 << "k";
      }
      else {
...
/// Output grid.
/// Prints vertices in decreasing z and y orders.
void output_grid
(ostream & out, const bool flag_grid_lines, const SGRID & scalar_grid)
{
  const int dimension = scalar_grid.Dimension();
  const AXIS_SIZE_TYPE * axis_size = scalar_grid.AxisSize();
  int output_width = 6;
  ...
  output_grid_dimensions(out, scalar_grid);
  out << endl;

  if (scalar_grid.Dimension() <= 1) {
    print_scalar(out, output_width, scalar_grid.Scalar(0));
    for (NUM_TYPE i = 1; i < scalar_grid.NumVertices(); ++i) {
      out << " ";
      print_scalar(out, output_width, scalar_grid.Scalar(i));
    }
    out << endl;
  }
  else {
    NUM_TYPE num_vertices_in_ridge;
    compute_num_vertices_in_grid_ridge
      (dimension, axis_size, 0, 1, num_vertices_in_ridge);
    IJK::ARRAY<VERTEX_INDEX> vlist(num_vertices_in_ridge);
    get_vertices_in_grid_ridge
      (dimension, axis_size, 0, 1, vlist.Ptr());

    for (NUM_TYPE i = 0; i < num_vertices_in_ridge; i++) {
      for (AXIS_SIZE_TYPE j1 = 0; j1 < axis_size[1]; j1++) {
        // Output vertices by decreasing z (and higher dimensions.)
        VERTEX_INDEX iv = vlist[num_vertices_in_ridge-1-i];
        // Output vertices by decreasing y.
        iv += (axis_size[1]-1-j1)*axis_size[1];
        out << setfill(' ');
        print_scalar(out, output_width, scalar_grid.Scalar(iv));
        ++iv;

        if (flag_grid_lines) { out << setfill('.'); }

        for (AXIS_SIZE_TYPE j0 = 1; j0 < axis_size[0]; j0++) {
          if (flag_grid_lines) { out << "."; }
          else { out << " "; }
          print_scalar(out, output_width, scalar_grid.Scalar(iv));
          ++iv;
        }
        ...
        if (flag_grid_lines && j1+1 < axis_size[1]) {
          out << setfill(' ');
          for (AXIS_SIZE_TYPE j0 = 0; j0 < axis_size[0]; j0++) {
            out << setw(output_width) << "|";
            out << " ";
          }
          out << endl;
        }
...
void check_input(const SGRID & scalar_grid)
{
  ...
  if (flag_cube_info) {
    ...
    if (cube_coord.size() != scalar_grid.Dimension()) {
      cerr << "Usage error. Number of coordinates in option -cc \"{coordinates}\""
           << endl;
      cerr << "  does not match scalar grid dimension." << endl;
      cerr << "  Scalar grid dimension is " << scalar_grid.Dimension()
           << "." << endl;
      cerr << "  Option -cc \"{coordinates}\" has "
           << cube_coord.size() << " coordinates." << endl;
      exit(10);
    }
    ...
    for (int d = 0; d < scalar_grid.Dimension(); d++) {
      ...
      if (scalar_grid.AxisSize(d) < TWO) {
        cerr << "Usage error.  Scalar grid contains no cubes.";
        cerr << "  Scalar grid axis " << d << " size is "
             << scalar_grid.AxisSize(d) << "." << endl;
        cerr << "  Option -cc cannot be used with a scalar grid that contains no cubes."
             << endl;
        exit(10);
      }
      
      if (cube_coord[d] < 0) {
        cerr << "Usage error. Cube coordinates must be non-negative." << endl;
        cerr << "  Coordinate " << d << " in -cc \"{coordinates}\" is "
             << cube_coord[d] << "." << endl;
        exit(10);
      }

      if (cube_coord[d]+1 >= scalar_grid.AxisSize(d)) {
        cerr << "Usage error. Illegal cube coordinate in -cc \"{coordinates}\"."
             << endl;
        cerr << "  Coordinate " << d << " in -cc \"{coordinates}\" is "
             << cube_coord[d] << "." << endl;
        cerr << "  Scalar grid axis " << d << " size is "
             << scalar_grid.AxisSize(d) << "." << endl;
        cerr << "  Cube coordinate must be less than "
             << scalar_grid.AxisSize(d)-1 << "." << endl;
        exit(10);
      }
    }
  }
...
void parse_command_line(int argc, char **argv)
{
  ...
  while (iarg < argc && argv[iarg][0] == '-') {
    ...
    case PRINT_GRID_PARAM:
      flag_print_grid = true;
      break;

    case PRINT_GRIDB_PARAM:
      flag_print_grid = true;
      flag_grid_lines = true;
      break;
