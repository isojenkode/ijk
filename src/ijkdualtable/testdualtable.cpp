...
typedef typename IJKDUALTABLE::VERTEX_CONNECTIVITY_AND_DEGREE_INFO<NUM_FACET_BITS16,short>
ISO_VERTEX_INFO;
...
typedef IJKDUALTABLE::DUAL_TABLE_VERTEX_CONNECTIVITY<int,ISO_VERTEX_INFO>
...
void check_isodual_ambig_cube_table(const int dimension);
...
int main(int argc, char **argv)
{
  try {
    ...
    if (flag_ivol) {
      ...
      if (flag_dim2) { check_isodual_ambig_cube_table(DIM2); }
      if (flag_dim3) { check_isodual_ambig_cube_table(DIM3); }
      ...
      cout << "Passed isodual ambig cube table check." << endl;
      cout << "Passed isodual vertex info check." << endl;
      cout << "Passed check of contains_two_opposite_ones and contains_two_opposite_zeros."
           << endl;
    }
  }
  catch (ERROR error) {
...
template <typename TABLE_TYPE>
void output_table_facet_info
(const TABLE_TYPE & table,
 const DUAL_TABLE_ISO_VERTEX_INFO & vinfo,
 const int it)
{
  std::vector<int> vlist;
  for (int kf = 0; kf < table.NumPolyFacets(); kf++) {
    vinfo.GetVerticesIncidentOnEdgesCrossingFacet(it, kf, vlist);
    if (vlist.size() > 0) {
      cout << "  Vertices incident on edges crossing facet " << kf
           << ":";
...
void output_isodualtable_ambig_vinfo(const int dimension)
{
  ...
  for (int it = 0; it < ambig_table.NumTableEntries(); it++) {
    ...
    if (ambig_table.NumIsoVertices(it) > 0) {
      ...
      output_ambig_info(it, ambig_table, cube_info);
      if (ambig_table.NumIsoVertices(it) > 1) 
        { output_table_facet_info(ambig_table, vinfo, it); }
    }
  }
  cout << endl;
}
...
void check_isodual_ambig_cube_table(const int dimension)
{
  ISODUAL_CUBE_TABLE_AMBIG isodual_table(dimension);
  IJK::PROCEDURE_ERROR error("check_isodual_ambig_cube_table");
