...
(const POLYMESH_TYPE & polymesh,
 const VERTEX_POLY_INCIDENCE_TYPE & vertex_poly_incidence);
void output_vertex_info
(const int dimension, const std::vector<int> & vertex_index);
...
(const char * input_filename, MESH_DATA & mesh_data);
void memory_exhaustion();
void parse_command_line(int argc, char **argv);
void usage_error_unknown_mesh_dimension(const MESH_DATA & mesh_data);
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    read_input_file(input_filename, mesh_data);
    ...
    identify_duplicates(polymesh_sorted, mesh_data.poly_data);
    
    if (flag_internal_poly || flag_internal_vert || flag_internal_edge) 
      { compute_facet_info(mesh_data); }

    if (io_info.flag_general_info) {
      if (!mesh_data.IsMeshDimensionSet())
        { usage_error_unknown_mesh_dimension(mesh_data); }
    ...
    if (manifold_flag) {
      ...
      if (flag_output_only_values) {
        ...
        if (flag_terse && flag_passed_tests) {
          ...
          if (mesh_data.mesh_dimension == DIM2 && output_filename != NULL &&
              !passed_all_manifold_tests && non_manifold_facet_vert.size() > 0) {
    ...
    if (flag_polyfile) {
      ...
      if (!mesh_data.IsMeshDimensionSet() && contains_edge_flag)
        { usage_error_unknown_mesh_dimension(mesh_data); }
    ...
    if (mesh_data.IsMeshDimensionSet() || !flag_polyfile) {
...
(const char * input_filename, MESH_DATA & mesh_data)
{
  ...
  read_poly_mesh_OFF(input_filename, mesh_data.dimension, vertex_coord,
                     mesh_data.num_vertices, mesh_data.polymesh.list_length,
                     mesh_data.polymesh.element, mesh_data.polymesh.first_element);
  mesh_data.poly_data.resize(mesh_data.polymesh.NumPoly());
  ...
  mesh_data.min_num_poly_vert = 
    *(min_element(mesh_data.polymesh.list_length.begin(),
                  mesh_data.polymesh.list_length.end()));
  mesh_data.max_num_poly_vert = 
    *(max_element(mesh_data.polymesh.list_length.begin(),
                  mesh_data.polymesh.list_length.end()));

  if (mesh_data.min_num_poly_vert == mesh_data.max_num_poly_vert) {

    num_vert_per_poly = mesh_data.min_num_poly_vert;

    if (!mesh_data.IsMeshDimensionSet()) {

      if (mesh_data.dimension == DIM3 &&
          num_vert_per_poly == 4) {
        // Unable to determine mesh dimension.
        // Do nothing.
      }
      else {
        if (num_vert_per_poly <= mesh_data.dimension+1) {
          // Assume poly are all simplices.
          mesh_data.SetMeshDimension(num_vert_per_poly-1);
        }
        else if (num_vert_per_poly == NUMV_PER_CUBE) {
          mesh_data.SetMeshDimension(DIM3);
          flag_cube_file = true;
        }
        else {
          mesh_data.SetMeshDimension(mesh_data.dimension);
        }
      }
    }
    ...
    if (num_vert_per_poly <= mesh_data.mesh_dimension+1) {
      ...
      simplex_vert = new VERTEX_INDEX[mesh_data.polymesh.element.size()];
      num_vert_per_simplex = num_vert_per_poly;
      num_simplices = num_poly;

      std::copy(mesh_data.polymesh.element.begin(),
                mesh_data.polymesh.element.end(),
                simplex_vert);
    ...
    if (flag_cube_file && mesh_data.IsMeshDimensionSet()) {
      ...
      if (numv_per_cube != num_vert_per_poly) {
        ...
        cerr << "Usage error.  Mismatch between input file of cubes and -mesh_dim argument."
             << endl;
    ...
    if (!mesh_data.IsMeshDimensionSet()) {
      mesh_data.SetMeshDimension(mesh_data.dimension-1);
...
(const int dimension, const std::vector<int> & vertex_index)
{
  for (int i = 0; i < vertex_index.size(); i++) {
    output_vertex_info(dimension, vertex_index[i]);
  }
}
...
void write_non_manifold_edges()
{
  ...
  if (!output_file.is_open()) {
    ...
    cerr << "  Unable to write non-manifold edges to file." << endl;
    return;
  }
  else {
    cout << endl;
    cout << "Writing non-manifold edges to file: " << output_filename << endl;
  }
...
// Set mesh dimension.
void MESH_DATA::SetMeshDimension(const int d)
{
  mesh_dimension = d;
...
void parse_command_line(int argc, char **argv)
{
  ...
  try {
    while (iarg < argc && argv[iarg][0] == '-') {
      ...
      case MESH_DIM_PARAM:
        {
          const int d = get_arg_int(iarg, argc, argv, error);
          mesh_data.SetMeshDimension(d);
        }
        iarg++;
        break;
      ...
      case VERTEX_PARAM:
        get_arg_multiple_arguments
          (iarg, argc, argv, vertex_index, error);
        iarg++;
      ...
      case POLY_PARAM:
        get_arg_multiple_arguments
          (iarg, argc, argv, poly_index, error);
        iarg++;
        poly_info_flag = true;
        io_info.flag_general_info = false;
        break;
