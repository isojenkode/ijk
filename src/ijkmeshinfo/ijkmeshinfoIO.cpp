...
void IJKMESHINFO::output_poly_vertices
...
void IJKMESHINFO::output_polytope_info
(const int dimension, const POLYMESH_TYPE & polymesh, 
 const COORD_TYPE * vertex_coord, const int poly_index)
{
  ...
  output_poly_vertices(dimension, polymesh, vertex_coord, poly_index);
}
...
(const int dimension, const POLYMESH_TYPE & polymesh, 
 ...
 const std::vector<int> & poly_index)
{
  for (int i = 0; i < poly_index.size(); i++) {
    output_polytope_info
      (dimension, polymesh, vertex_coord, poly_index[i]);
                         
  }
}


void IJKMESHINFO::output_polygon_info
(const int dimension, const POLYMESH_TYPE & polymesh, 
 const COORD_TYPE * vertex_coord, const int poly_index,
...
{
  cerr << "Usage: ijkmeshinfo [OPTIONS] {input file} [{output file}]" << endl;
  cerr << "OPTIONS:" << endl;
  cerr << "  [-mesh_dim {mdim}] [-reverse_orient]" << endl;
  cerr << "  [-vertex {vnum} | -vertex \"{v1} {v2}...\"]" << endl;
  cerr << "  [-simplex {snum}]" << endl;
  cerr << "  [-poly {pnum} | -poly \"{p1} {p2}...\"]" << endl;
  ...
  cerr << "  [-max_out <N>] [-o {output_filename}]" << endl;
  cerr << "  [-terse] [-no_tics] [-help]" << endl;
}
...
{
  cout << "Usage: ijkmeshinfo [OPTIONS] {input file}" << endl;
  cout << "  Print or plot information about output mesh." << endl;
  cout << "  Input file must have .off format." << endl;
  cout << endl;
  cout << "OPTIONS:" << endl;
  cout << "  -mesh_dim {mdim}:   Mesh dimension." << endl;
  cout << "  -reverse_orient:    Reverse the orientation of each mesh element." << endl;
  cout << "  -vertex {vnum}:     Print coordinates of vertex {vnum}."
       << endl;
  cout << "  -vertex \"{v1} {v2}...\": Print coordinates of vertices {v1}, {v2}, etc."
       << endl;  
  cout << "  -simplex {snum}:    Print vertices of simplex {snum}."
       << endl;
  cout << "  -poly {pnum}:       Print vertices of poly {pnum}."
       << endl;
  cout << "  -poly \"{p1} {p2}...\": Print vertices of poly {p1}, and poly {p2}, etc."
       << endl;  
  cout << "  -vlist:             Print list of vertex coordinates." << endl;
  cout << "     Prints only vertices with coordinates between min coord and max coord."
       << endl;
  cout << "  -vlist_min:         Print vertices with min values." << endl;
  cout << "     (Only implemented for hexahedral meshes.)" << endl;
  cout << "  -plist:             Print list of polygons/polytopes." << endl;
  cout << "     Print polygons with min/max angles, edge lengths, and"
       << endl
       << "       Jacobian determinants." << endl;
  cout << "     If -min_numv or -max_numv is set, prints only "
       << endl
       << "       polygons/polytopes with number of vertices between"
       << endl;
  cout << "       <min_numv> and <max_numv>." << endl;
  cout << "  -plist_vcoord:      Print list of polygons/polytopes"
       << endl
       << "                           and coordinates of vertices." 
       << endl;
  cout << "  -manifold:          Print manifold information." << endl;
  cout << "  -oriented_manifold: Print manifold and orientation information."
       << endl;
  cout << "  -check_facetI:      Check facet intersections." << endl
  ...
  cout << "  -selfI:             Print self intersections." << endl;
  cout << "                      Use spatial grid to eliminate many comparison tests." << endl;
  cout << "  -selfI:             Print self intersections by comparing all triangles pairs." << endl;
  cout << "                      Very slow, time consuming algorithm."
       << endl;
  cout << "  -containsv {vnum}:  Print list of polytopes containing vertex {vnum}."
       << endl;
  cout << "  -containse {end0} {end1}}:  Print list of polygons or simplices containing"
       << endl;
  cout << "                          edge ({end0},{end1})." << endl;
  cout << "  -minc \"min coord\":  Minimum coordinates." << endl;
  cout << "  -maxc \"min coord\":  Maximum coordinates." << endl;
  cout << "  -min_numv <N>:      Minimum number of poly vertices." << endl;
  cout << "  -max_numv <N>:      Maximum number of poly vertices." << endl;
  cout << "  -angle_le <A>:" << endl;
  cout << "     Report number of polygons with angles less than or equal to <A>"
       << endl
       << "       or number of tetrahedra with dihedral angles less than" 
       << endl
       << "       or equal to <A>." << endl;
  cout << "     Use with -plist to list polygons or tetrahedra with angles"
       << endl
       << "       or dihedral angles less than or equal to <A>." << endl;
  cout << "  -angle_ge <A>:" << endl;
  cout << "     Report number of polygons with angles greater than or equal to <A>" << endl
       << "       or number of tetrahedra with dihedral angles greater than" 
       << endl
       << "       or equal to <A>." << endl;
  cout << "     Use with -plist to list polygons or tetrahedra with angles or"
       << endl
       << "       dihedral angles greater than or equal to <A>." << endl;
  cout << "  -facet_angle_le <A>:" << endl;
  cout << "     Report number of tetrahedra with facet angles less than or equal to <A>." << endl;
  cout << "     Use with -plist to list tetrahedra with facet angles less than"
       << endl
       << "       or equal to <A>." << endl;
  cout << "  -facet_angle_ge <A>:" << endl;
  cout << "     Report number of tetrahedra with facet angles greater than or equal to <A>." << endl;
  cout << "     Use with -plist to list tetrahedra with facet angles greater than" << endl
       << "       or equal to <A>." << endl;
  cout << "  -elength_ratio_le <R>: " << endl;
  cout << "     Report number of polygons with ratio of shortest to longest"
       << endl
       << "       edge length less than or equal to <R>." << endl;
  cout << "     Use with -plist to list polygons with edge length ratios"
       << endl
       << "       less than or equal to <R>." << endl;
  cout << "  -list_dup:          List duplicate poly vertices or poly vertices" << endl;
  cout << "                           with identical coordinates." << endl;
  cout << "  -internal:          Report only min/max values for internal polytopes."
       << endl;
  cout << "  -internal_vert:     Report only min/max values at internal mesh vertices."
       << endl;
  cout << "  -internal_edge:     Report only min/max values at internal mesh edges."
       << endl;
  cout << "  -pJacobian:         Compute Jacobian values for each polytopes."
       << endl;
  cout << "  -vJacobian:         Compute Jacobian values at each vertex."
       << endl;
  cout << "  -out_values:        Output only values for -manifold option."
       << endl;
  cout << "     Output number of non-manifold vertices, number of non-manifold vertices"
       << endl
  ...
  cout << "  -out_min_angle:     Output minimum triangle angle." << endl;
  cout << "  -out_max_angle:     Output maximum triangle angle." << endl;
  cout << "  -out_min_jdet:      Output minimum Jacobian determinant." << endl
  ...
  cout << "  -out_max_jdet:      Output maximum Jacobian determinant." 
       << endl;
  cout << "  -plot_angles:       Create gnuplot (.gplt) files of min and max"
       << endl
       << "                          triangle angles." << endl;
  cout << "  -plot_edge_lengths: Create gnuplot (.gplt) files of min and max"
       << endl
       << "                          polytope edge lengths." << endl;
  cout << "  -plot_jacobian:     Create gnuplot (.gplt) files of min and max"
       << endl
       << "                          hexahedra Jacobian determinants." 
       << endl;
  cout << "  -plot_jshape:       Create gnuplot (.gplt) files of min and max"
       << endl
       << "                          hexahedra Jacobian shave values." 
       << endl;
  cout << "  -report_deep:       Report only boundary facets at least distance 1"
       << endl
       << "                          from bounding box." << endl;
  cout << "  -for_each_type:     Reports angles separately for triangles, "
       << endl
       << "                          quadrilaterals, pentagons, ..." << endl;
  cout << "  -max_out <N>:       Limit number of polytopes in output lists to <N>." << endl;
  cout << "  -o {output_filename}:  Output filename." << endl
       << "                      Used only with -manifold or -oriented_manifold to report"
       << endl
       << "                          non-manifold edges." << endl;
  cout << "  -terse:             Terse output.  Affects mesh information"
       << endl
       << "                          and -manifold and -containsv and -selfI options."
       << endl;
  cout << "  -no_tics:           Do not output tics to indicate program still running."
       << endl
