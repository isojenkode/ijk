/*!
 *  @file ijksnapmc.cpp
 *  @brief Snap MC isosurface generation.
 *  - Version 0.6.0
 */
...
/*!
 *  @brief SnapMC Algorithm.
 *  - Improves quality of isosurface simplices by "snapping" isosurface vertices
 *    to grid vertices for isosurface triangulation construction.
 *  - Isosurface vertices constructed by SnapMC are a subset
 *    of the isosurface vertices constructed by regular Marching Cubes.
 */
...
/*!
 *  @brief Extract isosurface using Marching Cubes algorithm.
 *  - Returns list of isosurface simplex vertices
 *    and list of isosurface vertex coordinates.
 *  @param scalar_grid Scalar grid data.
 *  @param isotable Hypercube isosurface lookup table for given dimension.
 *  @param isovalue Isosurface scalar value.
 *  @param snap_value Snap isosurface vertices within distance snap_value
 *         of grid vertices. 0 = no snapping. 0.5 = max snapping.
 *    @pre snap_value in range [0.0, 0.5].
 *  @param nep_num_dup Controls duplicate isosurface patches.
 *  @param simplex_vert[] List of isosurface simplex vertices.
 *         simplex_vert[dimension*is+k] = k'th vertex of simplex is.
 *  @param vertex_coord[] List of isosurface vertex coordinates.
 *  @param vertex_coord[dimension*iv+k] k'th coordinate of vertex iv.
 *  @param merge_data Internal data structure for merging identical edges.
 *  @param snap_info Information about running time and grid cubes and edges.
 */
void IJKSNAPMC::snapMC
 ...
 *  @param vertex_coord[dimension*iv+k] k'th coordinate of vertex iv.
 *  @param merge_data Internal data structure for merging identical edges.
 *  @param minmax SnapMC min and max of regions
 *  @param snap_info Information about running time and grid cubes and edges.
 */
void IJKSNAPMC::snapMC
(const MC_SCALAR_GRID_BASE & scalar_grid, const NEP_ISOSURFACE_TABLE & isotable,
...
namespace {
    ...
    // snap scalar values in upper facet
  {
    ...
    IJK::ARRAY<VERTEX_INDEX> axis_increment(scalar_grid.Dimension());
    ...
    if (minmax.Min(iregion) < isovalue &&
      ...
      if (is_full[iregion]) {
        for (VERTEX_INDEX j = 0; j < max_num_cubes_in_region; j++) {
          ...
          snap_incident_edges
            (dimension, axis_increment.PtrConst(), scalar, v1, isovalue,
  ...
  for (int i = 0; i < numv; i++) {
    ...
    if (k == VERTEX_OFFSET) {
      if (scalar_snap[v0] == scalar[v0]) {
        ...
        for (int d = 0; d < dimension; d++)
          { coord[i*dimension+d] = COORD_TYPE(coord0[d]); }
      }
      else {
  ...
  // snap_vlist may contain some extra cubes
{
  ...
  VERTEX_INDEX num_cube_vertices = isotable.Polytope().NumVertices();
  ...
  compute_cube_vertex_increment
    (dimension, axis_increment.PtrConst(), vertex_increment.Ptr());
