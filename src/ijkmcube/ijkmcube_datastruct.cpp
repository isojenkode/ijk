/*!
 *  @file ijkmcube_datastruct.cpp
 *  @brief ijkmcube data structures.
 *  - Version 0.6.0
 */
...
void POLY_ISOTABLE::ComputeAmbiguityInformation()
{
  if (cube.IsTableAllocated())
    { ambig_cube.SetAmbiguityTable(cube.Polytope()); };

  if (pyramid.IsTableAllocated())
    { ambig_pyramid.SetAmbiguityTable(pyramid.Polytope()); };
}
...
{
  this->high_resolution_region.clear();

  for (unsigned int i = 0; i < high_resolution_region.size(); i++) {
    this->high_resolution_region.push_back(high_resolution_region[i]);
  }
...
void MC_DATA::SetMultiresGrid(const int num_resolution_levels)
{
  ...
  multires_grid->SetRegion2Multires(high_resolution_region, high_res_scale);
...
/// Set flag_interval_volume
void MC_DATA::SetFlagIntervalVolume(const bool flag)
{
  flag_interval_volume = flag;
}

/// Set use_cube_list
void MC_DATA::SetUseCubeList(const bool flag)
{
  use_cube_list = flag;
}
...
bool MC_DATA::Check(IJK::ERROR & error) const
{
  ...
  if (UseNEP() || Snap()) {
	...
	(isotable.cube_nep, IJKMCUBE_TABLE::BASE3, error)) {
      return(false);
    }
  }
  else {
    ...
    if (FlagIntervalVolume()) {
      if (!check_isotable_encoding
          (isotable.cube, IJKMCUBE_TABLE::BASE3, error)) {
        return(false);
      }
    }
    else {
      if (!check_isotable_encoding
          (isotable.cube, IJKMCUBE_TABLE::BINARY, error)) {
        return(false);
      }
    }
  }
  ...
  if (IsosurfaceTopology() == ASYMPTOTIC_DECIDER_TOPOLOGY ||
    ...
    if (!check_isotable_encoding
	(isotable.pyramid, IJKMCUBE_TABLE::BINARY, error)) {
      return(false);
    }
  ...
  if (IsosurfaceTopology() == SADDLE_TOPOLOGY ||
    ...
    if (!check_isotable_encoding
	(isotable.simplex, IJKMCUBE_TABLE::BINARY, error)) {
      return(false);
    }
  }
...
// Set is_in_facet[it] = flag, for all table entries it.
void NEP_ISOSURFACE_TABLE::SetIsInFacet(const bool flag)
{
  for (IJKMCUBE_TABLE::TABLE_INDEX it = 0; it < NumTableEntries(); it++)
    { is_in_facet[it] = flag; };
}
...
void NEP_ISOSURFACE_TABLE::SetNumTableEntries(const int num_table_entries)
{
  ...
  containing_facet = new IJKMCUBE_TABLE::FACET_INDEX[num_table_entries];
