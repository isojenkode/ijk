 ...
 *  @brief Data structure definitions for ijkmcube.
 *  - Version 0.6.0
 */
...
namespace IJKMCUBE {
  ...
  <GRID_CUBEV_BITSET_SIZE,int,AXIS_SIZE_TYPE,VERTEX_INDEX,VERTEX_INDEX>
    MC_GRID_PLUS;             ///< Regular grid.
  typedef IJK::GRID_SPACING<COORD_TYPE, MC_GRID_PLUS>
    MC_GRID;                  ///< Regular grid and spacing information.
  ...
  typedef IJK::GRID<int, AXIS_SIZE_TYPE, VERTEX_INDEX, VERTEX_INDEX>
    MINMAX_GRID_BASE;         ///< Base for Marching Cubes grid.

  /// @brief Marching Cubes structures for computing minimum/maximum in regions.
  typedef IJK::MINMAX_REGIONS<MINMAX_GRID_BASE, SCALAR_TYPE>
    MINMAX_REGIONS;           
  ...
  /*!
   *  @brief NEP isosurface lookup table.
   *  - Includes information about isosurface patches contained 
   *    on polyhedron facets.
   */
  class NEP_ISOSURFACE_TABLE:public ISOSURFACE_TABLE {

  protected:

    /// is_in_facet[k] = true if iso patch is totally contained 
    ///   in a cube facet.
    bool * is_in_facet;

    /// containing_facet[k] = facet containing iso patch.
    IJKMCUBE_TABLE::FACET_INDEX * containing_facet;
  ...
  public:
    ...
    /// @brief Return true if iso patch at table entry it is totally contained in a cube facet.
    /// @param it Index of table entry containing iso patch.
    bool IsInFacet(const TABLE_INDEX it) const
      { return(is_in_facet[it]); };

    /// Return facet containing isopatch.
    IJKMCUBE_TABLE::FACET_INDEX ContainingFacet(const TABLE_INDEX it) const
    ...
    void SetContainingFacet(const TABLE_INDEX it, 
                            const IJKMCUBE_TABLE::FACET_INDEX jf)
  ...
  public:
    ...
    /// True if generating an interval volume instead of an isosurface.
    ...
    bool use_octree;
    bool use_nep;
    bool use_cube_list;            ///< Extract non-empty cubes to list.
    bool use_multires;

    /// @brief If true, use SnapMC algorithm to construct isosurface.
    bool flag_snap;

    // *** CHANGE TO flag_cube_containing_simplex
    ...
    /*!
     * @brief Parameter for processing duplicate isosurface triangles.
     * \anchor nepNumDupDoc
     *      - If 0, all duplicate isosurface triangles are deleted.
     *        (Duplicate triangles are replace by 0 triangles.)
     *      - If 2, isosurface may contain duplicate isosurface triangles.
     *        (Allow 2 identical triangles in isosurface.)
     */
    int nep_num_dup;

    /// Return default number for nep_num_dup.
    int DefaultNEPnumDup() const
    { return 2; }

    // Multiresolution parameters
    int num_resolution_levels;
    std::vector<GRID_BOX> high_resolution_region;
  ...
  public:
    ...
    bool UseNEP() const             /// Return NEP flag.
      { return(use_nep); };
    bool UseCubeList() const        /// Return use_cube_list flag.
      { return(use_cube_list); };
  };


  /// Input data to Marching Cubes and related algorithms
  class MC_DATA:public MC_DATA_FLAGS {
  ...
  public:
    ...
    void SetFlagIntervalVolume      /// Set flag_interval_volume.
      (const bool flag);
    ...
    void SetUseCubeList(const bool flag);    ///< Set use_cube_list.
    void SetHighResolutionRegions        /// Set high resolution regions.
      (const std::vector<GRID_BOX> & high_resolution_region);
    void SetCubeContainingSimplexFlag    /// Set cube_containing_simplex flag.
      (const bool flag);
    void SetMergeEdgesParameters         /// Set merge edges parameters.
       (const MERGE_EDGES_PARAMETERS & param)
     { merge_edges_parameters = param; }

    /// @brief Copy, subsample or supersample scalar grid.
    /// @pre flag_subsample and flag_supersample are not both true.
    void SetScalarGrid
      (const MC_SCALAR_GRID_BASE & scalar_grid2, 
    ...
    MESH_EDGE_REPRESENTATION 
      EdgeRepresentation() const  /// Return type of edge representation.
    ...
    int NEPNumDup() const           /// Return nep_num_dup
      { return(nep_num_dup); };
    bool FlagIntervalVolume() const /// Return flag_interval_volume
      { return(flag_interval_volume); };

    const MERGE_EDGES_PARAMETERS & MergeEdgesParameters() const
    { return merge_edges_parameters; }
  ...
  protected:
    ...
    /// @briefNumber of ambiguous cubes with i saddles.
    VERTEX_INDEX * num_cubes_with_saddle; 
  ...
  template <class DTYPE, class CTYPE, class STYPE>
    ...
    /// @brief scalar[i] = scalar value of i'th mesh vertex.
