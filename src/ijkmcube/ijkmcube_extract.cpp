...
/*
  ...
  Copyright (C) 2006-2025 Rephael Wenger
...
namespace {
    ...
    // constructor
  {
    ...
    assert(isotable.Polytope().NumVertices() == 
           compute_num_cube_vertices(isotable.Dimension()));

    dimension = isotable.Dimension();
    num_cube_vertices = isotable.Polytope().NumVertices();
...
namespace {
  ...
  /*!
   *  @brief For use in determining isosurface vertices on a facet.
   *  - Used in computing isosurface simplices lying on grid boundaries
   *    with NEP isosurface lookup table.
   */
    ...
    VERTEX_INCREMENT(axis_size, isotable, isotable_type)
  {
    ...
    if (isotable_type != NEP) {
      error.AddMessage
        ("Programming error. FACET_VERTEX_INCREMENT can only be used with NEP isotable.");
      throw error;
    }
    ...
    compute_facet_vertex_increment
      (isotable.Polytope(), orth_axis, side, axis_size,
  ...
  {
    const int dimension = isotable.Dimension();
    IJK::ARRAY<VERTEX_INDEX> cube_increment2(num_cube_vertices);
...
namespace {
  ...
  /*!
   *  @brief Increment for computing the index of an entry in an NEP table.
   *  - Positive(i) is the table index of a cube
   *    with positive vertex i (and all other vertices negative.)
   *  - Equals(i) is the table index of a cube
   *    with vertex i set labelled equals (and all other vertices negative.)
   *  - Indices of other entries into the NEP table can be computed
   *    by summing values of Positive(i) and Equals(i)
   *    for appropriate i.
   */
...
namespace {
  ...
  {
    IJK::ARRAY<SCALAR_TYPE> b(dimension);
    IJK::ARRAY<SCALAR_TYPE> x(dimension);
   ...
   int & num_positive, int & num_negative)
  {
    ...
    const int num_cube_vertices = cube_isotable.Polytope().NumVertices();
    const int num_cube_facets = cube_isotable.Polytope().NumFacets();
    const int num_facet_vertices = num_cube_vertices/2;
    IJK::ARRAY<SCALAR_TYPE> base_scalar(num_facet_vertices);
    ...
    for (int jf = 0; jf < num_cube_facets; jf++) {
      ...
      if (cube_ambig.IsFacetAmbiguous(it, jf)) {
        ...
        for (int k = 0; k < num_facet_vertices; k++) {
          VERTEX_INDEX kv = cube_isotable.Polytope().FacetVertex(jf, k);
        ...
        bool is_ge_isovalue = 
          ...
          (dimension-1, base_scalar.PtrConst(), num_facet_vertices, isovalue,
           parity_sign);
  ...
  /// Get scalar values of polytope vertices.
  ...
  /*!
   *  @brief Extract isosurface simplices in cube.
   ...
   *  @param isotable Cube (hypercube) isosurface table for given dimension.
   *  @param isovalue Isosurface scalar value.
   *  @param iv0 pPimary cube vertex (cube vertex with lowest coordinates).
   *  @param increment[d] Vertex increment in direction d.
   *  @param[out] iso_simplices[] Array of isosurface simplex vertices.
   *    - iso_simplices[dimension*is+k] = 
   *      grid edge containing k'th vertex of simplex is.
   *  @param[out] num_simplices Number of simplices extracted.
   */
  ...
  (const SCALAR_TYPE * scalar, const ISOSURFACE_TABLE & isotable,
   ...
   int & num_simplices)
  {
    const int num_cube_vertices = isotable.Polytope().NumVertices();
  ...
  /// @brief Extract isosurface simplices from ambiguous cube configuration.
  ...
  /// Extract isosurface simplices from ambiguous cube configuration
  /// @param ambiguity_sign = If true, use table entry it.
  ///   - Otherwise, use complement of table entry it.
  ...
  /// Extract isosurface simplices from ambiguous polytope configuration
   ...
   MC_MESH_VERTEX_LIST & new_mesh_vertices)
  {
    ...
    add_coord(dimension, coord0.PtrConst(), offset_coord, coord2.Ptr());
    new_mesh_vertices.SetCoord(k, coord2.PtrConst());

    SCALAR_TYPE s = 
      SCALAR_TYPE
      (multilinear_interpolate_scalar
       (dimension, offset_coord, num_cube_vertices, 
        cube_scalar.PtrConst()));
   ...
   int & num_simplices, SCALAR_INFO & ambiguous_info)
  {
    ...
    const int num_cube_vertices = 
      poly_isotable.cube.Polytope().NumVertices();
    const int num_cube_facets = 
      poly_isotable.cube.Polytope().NumFacets();
    IJK::ARRAY<int> decider_sign(num_cube_facets);
    ...
    if (poly_isotable.ambig_cube.NumAmbiguousFacets(it) == 0) {
      ...
      compute_signs_of_facet_deciders
        (scalar_grid, poly_isotable.cube, poly_isotable.ambig_cube,
         isovalue, it, iv0, increment, parity_sign, decider_sign.Ptr(),
   ...
   int & num_simplices, SCALAR_INFO & ambiguous_info)
  {
    ...
    const int num_cube_vertices = cube_isotable.Polytope().NumVertices();
    const int num_facet_vertices = num_cube_vertices/2;
    IJK::ARRAY<SCALAR_TYPE> facet_scalar(num_facet_vertices);

    for (int k = 0; k < num_facet_vertices; k++) {
      VERTEX_INDEX kv = cube_isotable.Polytope().FacetVertex(jfacet, k);
      facet_scalar[k] = cube_scalar[kv];
    }

    bool decider_sign = 
      is_cube_decider_ge_isovalue
      (dimension-1, facet_scalar.PtrConst(), num_facet_vertices, isovalue,
       parity_sign);
   ...
   int & num_simplices, SCALAR_INFO & ambiguous_info)
  {
    const int dimension = scalar_grid.Dimension();
    const int num_cube_vertices = cube_isotable.Polytope().NumVertices();
    IJK::ARRAY<COORD_TYPE> min_coord(dimension);
    IJK::ARRAY<COORD_TYPE> max_coord(dimension);
    IJK::ARRAY<COORD_TYPE> coord0(dimension);
    IJK::ARRAY<COORD_TYPE> coord1(dimension);
    IJK::ARRAY<COORD_TYPE> coord2(dimension);
    IJK::ARRAY<SCALAR_TYPE> cube_scalar(num_cube_vertices);
    IJK::ARRAY<SCALAR_TYPE> new_cube_scalar(num_cube_vertices);
    IJK::ARRAY<VERTEX_INDEX> new_cube_vertex(num_cube_vertices);
    IJK::PROCEDURE_ERROR error("extract_from_subcubes_in_cube");

    for (int d = 0; d < dimension; d++) {
      COORD_TYPE c0 = coordA[d];
      COORD_TYPE c1 = coordB[d];
      min_coord[d] = std::min(c0, c1);
      max_coord[d] = std::max(c0, c1);
    }
    ...
    for (int i = 0; i < num_cube_vertices; i++) {
      ...
      long mask = 1L;
      for (int d = 0; d < dimension; d++) {
        if ((i & mask) == 0)  { coord1[d] = min_coord[d]; }
        else { coord1[d] = max_coord[d]; }

        mask = (mask << 1L);
      }

      add_coord(dimension, coord0.PtrConst(), coord1.PtrConst(), 
                coord2.Ptr());
      new_mesh_vertices.SetCoord(k+i, coord2.PtrConst());

      SCALAR_TYPE s = 
        SCALAR_TYPE 
        (multilinear_interpolate_scalar
         (dimension, coord1.PtrConst(), num_cube_vertices, 
          cube_scalar.PtrConst()));

      new_mesh_vertices.SetScalar(k+i, s);
    }

    for (int jf = 0; jf < cube_isotable.Polytope().NumFacets(); jf++) {
      const int d = jf/2;
      ...
      compute_isotable_index
        (new_cube_scalar.PtrConst(), isovalue, num_cube_vertices, new_it);

      if (cube_ambig.IsAmbiguous(new_it)) {
        // Only facet jf can be ambiguous.
        extract_from_cube_with_ambiguous_facet
          (new_cube_scalar.PtrConst(), new_cube_vertex.PtrConst(), 
           cube_isotable, isovalue, new_it, jf, parity_sign,
           iso_simplices, endpoint, new_mesh_vertices,
           num_simplices, ambiguous_info);
      }
      else {
        add_iso_simplices
          (cube_isotable, new_it, 0, new_cube_vertex.PtrConst(),
           iso_simplices, endpoint, num_simplices);
      }
    }
   ...
   int & num_simplices, SCALAR_INFO & ambiguous_info)
  {
    ...
    compute_signs_of_facet_deciders
      (scalar_grid, poly_isotable.cube, poly_isotable.ambig_cube, isovalue, it,
       iv0, increment, parity_sign, decider_sign.Ptr(),
       num_positive, num_negative);
   ...
   int & num_simplices, SCALAR_INFO & ambiguous_info)
  {
    ...
    SCALAR_TYPE a = 0;
    compute_saddle_3D
      (cube_scalar.PtrConst(), parity_sign, a, trilinear_midpoint, 
       saddle_offset, num_saddles, 0.0001);
  ...
  /*!
   *  @brief Extract isosurface simplices from cube with primary vertex iv0.
   ...
   *        scalar[x0 + x1*grid_length[0] + 
   *                    x2*grid_length[0]*grid_length[1] + ...]
   *  @param cube_isotable Hypercube isosurface table for given dimension
   *  @param isovalue Isosurface scalar value
   *  @param iv0 Primary cube vertex (cube vertex with lowest coordinates)
   *  @param increment[d] Vertex increment in direction d.
   *  @param[out] iso_simplices[] Vector of isosurface simplex vertices
   *    - iso_simplices[dimension*is+k] = 
   *        grid edge containing k'th vertex of simplex is.
   *  @param[out] num_simplices Number of simplices extracted.
   */
  ...
  /*!
   *  @brief Extract isosurface simplices from cube with primay vertex iv0.
   *  - Use cube center to resolve ambiguities.
   *  - Note: Make this inline for faster execution.
   *  - Returns list representing isosurface simplices
   *  @param scalar[] Array of scalar values
  ...
  /*!
   *  @brief Extract isosurface simplices from pyramid with primary vertex iv0.
   *  - Use linear topology to resolve ambiguities.
   *  - Note: Make this inline for faster execution.
   *  - Returns list representing isosurface simplices.
   *  @param pyramid_scalar[i] = Scalar value of pyramid vertex \a i.
   *  @param pyramid_vertex[i] = Index of grid vertex corresponding to pyramid vertex \a i.
   *  @param iso_simplices Vector of isosurface simplex vertices.
   *    - iso_simplices[dimension*is+k] = k'th vertex of isosurface simplex is.
   *  @param endpoint[] Endpoints of edges containing isosurface vertices.
   *    - (endpoint[2*i], endpoint[2*i+1]) = grid edge containing isosurface
   *      vertex i.
   *  @param num_simplices Number of simplices extracted.
   */
   ...
   int & num_simplices, SCALAR_INFO & ambiguous_info)
  {
    ...
    const int num_pyramid_vertices = 
      pyramid_isotable.Polytope().NumVertices();
   ...
   int & num_simplices, SCALAR_INFO & ambiguous_info)
  {
    ...
    const int num_cube_facet_vertices = num_cube_vertices/2;
    const int num_cube_facets = poly_isotable.cube.Polytope().NumFacets();
    const int num_pyramid_vertices = num_cube_facet_vertices+1;
    IJK::ARRAY<SCALAR_TYPE> pyramid_scalar(num_pyramid_vertices);
    IJK::ARRAY<VERTEX_INDEX> pyramid_vertex(num_pyramid_vertices);
    ...
    for (int i = 0 ; i < num_cube_facets; i++) {
      for (int j = 0; j < poly_isotable.cube.Polytope().NumFacetVertices(i);
           j++) {
        VERTEX_INDEX jv = poly_isotable.cube.Polytope().FacetVertex(i, j);
      ...
      extract_from_pyramid_linear_topology
        (pyramid_scalar.PtrConst(), pyramid_vertex.PtrConst(), 
  ...
  /*!
   *  @brief Extract isosurface simplices in cube with primary vertex iv0
   ...
   *        scalar[x0 + x1*grid_length[0] +
   *                   x2*grid_length[0]*grid_length[1] + ...]
   *  @param isotable Hypercube isosurface table for given dimension.
   *  @param isovalue Isosurface scalar value.
   *  @param iv0 Primary cube vertex (cube vertex with lowest coordinates).
   *  @param increment[d] Vertex increment in direction d.
   *  @param table_index_increment[] Table index increments.
   *  @param[out] iso_simplices[] Vector of isosurface simplex vertices.
   *    - iso_simplices[dimension*is+k] =
   *        grid edge containing k'th vertex of simplex is.
   *  @param[out] num_simplices Number of simplices extracted.
   ...
   *    using isotable which differentiates between scalar values
   *    less than (negative), equals or greater than (positive) the isovalue.
   *  - Does not extract simplices for cubes on grid boundary.
   *  - Instead, returns such cubes in list in_facet_cube[].
   *  @param scalar Array of scalar values
   *    - point (x0,x1,x2,...) has scalar value
   *        scalar[x0 + x1*grid_length[0] +
   *                   x2*grid_length[0]*grid_length[1] + ...]
   ...
   *        grid edge containing k'th vertex of simplex is.
   *  @param[out] num_simplices Number of simplices extracted.
   *  @param[out] in_facet_cube List of cubes whose isosurface patches 
   *     lie in a grid facet.
   */
  inline void extract_from_cube_nep
   ...
   int & num_simplices, std::vector<VERTEX_INDEX> & in_facet_cube)
  {
    ...
    if (isotable.IsInFacet(it)) {
      // Isosurface patch is totally inside cube facet.
      // Do not extract iso simplices.
      num_simplices = 0;
  ...
  /*!
   *  @brief Extract iso simplices using the asymptotic decider algorithm.
   *  - Produces linear topology on 3D cube facets but not inside cube.
   *  - Encapsulating this code in a separate function produces faster run time.
   */
   ...
   SCALAR_INFO & ambiguous_info)
  {
    ...
    const VERTEX_INDEX num_cube_vertices = 
      poly_isotable.cube.Polytope().NumVertices();
    IJK::ARRAY<int> parity_sign(num_cube_vertices);
      ...
      for (VERTEX_INDEX iv = facet_vlist[j]; 
        ...
        extract_from_cube_adecider
          (scalar_grid, poly_isotable, isovalue, iv, 
           increment, parity_sign.PtrConst(),
           iso_simplices, endpoint, new_mesh_vertices,
           num_simplices, ambiguous_info);
      };
    }
  ...
  // Extract isosurface patches which lie on cube facets.
  // *** CHANGE TO extract_iso_patches_in_cube_facets
  // @param in_facet_cube List of cubes whose iso patches lie on cube facets.
   ...
   MCUBE_INFO & mcube_info)
  {
    ...
    IJK::ARRAY<VERTEX_INDEX> facet_increment(numf);
    std::vector<VERTEX_INDEX> facet_list;
    std::vector<VERTEX_INDEX> sorted_facet_list;
    std::vector<VERTEX_INDEX> facet_list2;

    compute_facet_increment(isotable.Polytope(), increment.AxisSize(),
                            facet_increment.Ptr());

    for (unsigned int icube = 0; icube < in_facet_cube.size(); icube++) {
      // compute cube index
      ...
      // jf = index of facet containing the isopatch.
      // kf = lowest vertex of facet containing the isopatch.
   ...
   MCUBE_INFO & mcube_info)
  {
    ...
    // handle list of cubes whose isosurface patches lie totally in cube facets.
...
// - Returns list representing vertices of isosurface simplices.
// param iso_simplices[] Vector of isosurface simplex vertices
//   iso_simplices[dimension*is+k] =
//     grid edge containing k'th vertex of simplex is.
 ...
 MCUBE_INFO & mcube_info)
{
  ...
  VERTEX_INCREMENT increment(axis_size, isotable, IJKMCUBE::BINARY);
...
// Extract isosurface simplices and cube information.
// - Returns list representing isosurface simplices
//   and list of cubes containing each simplex.
...
// Extract isosurface simplices.
// - Return edges containing isosurface simplex vertices.
void IJKMCUBE::extract_iso_simplices
(const MC_SCALAR_GRID_BASE & scalar_grid, const ISOSURFACE_TABLE & isotable,
 const SCALAR_TYPE isovalue, std::vector<ISO_VERTEX_INDEX> & iso_simplices,
 ...
 MCUBE_INFO & mcube_info)
{
  ...
  if (isosurface_topology == ASYMPTOTIC_DECIDER_TOPOLOGY ||
    ...
    if (!check_isotable_encoding
        (poly_isotable.pyramid, IJKMCUBE_TABLE::BINARY, error)) 
      { throw error; };
  }
  ...
  if (isosurface_topology == SADDLE_TOPOLOGY ||
    ...
    if (!check_isotable_encoding
        (poly_isotable.simplex, IJKMCUBE_TABLE::BINARY, error)) 
      { throw error; };
  }
 ...
 MCUBE_INFO & mcube_info)
{
  ...
  VERTEX_INCREMENT increment(axis_size, poly_isotable.cube, IJKMCUBE::BINARY);
 ...
 std::vector<ISO_VERTEX_INDEX> & iso_simplices, MCUBE_INFO & mcube_info)
{
  ...
  case 1:
    // NOT YET IMPLEMENTED
    // DEFAULTS TO case 2
    // case 1: Replace duplicate isosurface triangles with a single triangle.
    //   Resulting surface would not be a manifold.
 ...
 std::vector<ISO_VERTEX_INDEX> & iso_simplices, int & num_non_empty_cubes)
{
  ...
  for (int d = 0; d < dimension; d++) {
    for (int side = 0; side < 2; side++) {
      ...
      get_facets_in_grid_facet(dimension, axis_size, d, side, vlist.Ptr());
  ...
  // mcube_info = mcube information
{
  ...
  if (isotable.Encoding() == IJKMCUBE_TABLE::BINARY) {
  ...
  else if (isotable.Encoding() == IJKMCUBE_TABLE::BASE3) {
...
// Extract isosurface simplices from list.
// - Use negative, equals, positive (nep) isosurface lookup table.
...
// **************************************************
// CLASS MULTI-RESOLUTION POLYTOPE
// **************************************************
  ...
  // Precondition: cube vertices are the corners of a 3x3x...x3 grid
{
  ...
  if (dimension > 0) {
    ...
    for (int ifacet = 0; ifacet < 2*dimension; ifacet++) {
      ...
      for (unsigned int j = 0; j < facet_plist.size(); j++) {
  ...
  // Precondition: cube vertices are the corners of a 3x3x...x3 grid
{
  ...
  // Type A polytope have dimension 2 or greater
  if (dimension > 1) {
    const int num_cube_facet_vertices = num_cube_vertices/2;
    MULTIRES_POLY poly(dimension, num_cube_facet_vertices+1,
                       cube_orientation);
...
namespace {
    ...
    //     at least number of cubes in facet0
  {
    ...
    IJK::ARRAY<VERTEX_INDEX> coord0(dimension);
    ...
    //   Precondition: Must be positive.
  {
    ...
    VERTEX_INCREMENT increment
      (axis_size, poly_isotable.cube, IJKMCUBE::BINARY);
...
namespace {
  ...
  void partition_multires(MULTIRES_GRID & multires_grid)
  {
    ...
    while (stack.size() > 0) {
      ...
      compute_coord(region.primary, dimension, axis_size, coord.Ptr());
      compute_cubic_region_corner_increment
        (dimension, axis_size, region.length+1, cube_increment.Ptr());

      if (is_cube_contained(dimension, axis_size, coord.PtrConst(), 
                            region.length)) {
        ...
        for (int i = 0; i < num_cube_vertices; i++) {
          if (is_cube_vertex_contained
              (dimension, axis_size, coord.PtrConst(), 
               half_region_length, i)) {
  ...
  void propagate_multires(MULTIRES_GRID & multires_grid)
  {
    ...
    while (length <= max_cube_length) {
      ...
      compute_face_centers(dimension-1, dimension, axis_size, length,
                           0, axis_increment.PtrConst(), &(center_list[0]));
      ...
      for (int k = 1; k < dimension; k++) {
        for (int ifacet = 0; ifacet < dimension; ifacet++) {
          ...
          compute_face_centers
            (k, dimension, axis_size, length,
             ifacet, axis_increment.PtrConst(), &(center_list[0]));
