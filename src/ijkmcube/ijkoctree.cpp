...
{
  IJK::ARRAY<AXIS_SIZE_TYPE> axis_range(dimension);
...
void OCTREE::SetMinMax(const IJKOCTREE::MINMAX_REGIONS & minmax_regions)
{
  const MASK_TYPE ZERO = 0;
  const MASK_TYPE ONE = 1;
  const KEY_TYPE num_keys = bon_octree_table.NumKeys();
  IJK::ARRAY<VERTEX_INDEX> axis_increment(dimension);
  IJK::ARRAY<VERTEX_INDEX> region_increment(num_keys*num_keys);
  ...
  // constructor
{
  ...
  const MASK_TYPE ONE = 1;
  IJK::ARRAY<VERTEX_INDEX> axis_increment(dimension);
  IJK::ARRAY<MASK_TYPE> direction_mask(dimension);
