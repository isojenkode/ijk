/*!
 *  @file ijkMCtable_ambig.cpp
 *  @brief Class containing a table of Marching Cubes isosurface patches in a given polyhedron.
 *  - Version 0.6.0
 */
...
{
  IJKMCUBE_TABLE::ISOTABLE_POLY cube(dimension);
...
bool IJKMCUBE_TABLE::is_poly_ambiguous
(const IJKMCUBE_TABLE::ISOTABLE_POLY_BASE & poly, 
...
bool IJKMCUBE_TABLE::is_facet_ambiguous
(const IJKMCUBE_TABLE::ISOTABLE_POLY_BASE & poly,
 const IJKMCUBE_TABLE::FACET_INDEX ifacet, const int * vertex_sign)
{
  const int numv = poly.NumVertices();
...
/// @param vertex_sign[i] = Sign of isosurface vertex i.
int IJKMCUBE_TABLE::compute_num_connected
(const IJKMCUBE_TABLE::ISOTABLE_POLY_BASE & poly,
...
/// @param vertex_sign[i] = Sign of isosurface vertex i.
void IJKMCUBE_TABLE::compute_ambiguous_facets
(const ISOTABLE_POLY_BASE & poly, const int * vertex_sign,
...
/// @brief Return number of vertices connected by edges in facet jf
/// to vertex iv with same sign as iv.
/// @param jf = Facet index.
/// @param iv = Vertex index.  Precondition: Vertex iv is in facet jf.
/// @param vertex_sign[i] = Sign of isosurface vertex i.
int IJKMCUBE_TABLE::compute_num_connected_in_facet
(const IJKMCUBE_TABLE::ISOTABLE_POLY_BASE & poly,
