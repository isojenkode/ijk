/*!
 *  @file ijkmcube_util.cpp
 *  @brief Utility functions for ijkmcube
 *  - Version 0.6.0
 */
  ...
  //               isotable.NumIsosurfaceVertices()
{
  ...
  assert(isotable.Encoding() == IJKMCUBE_TABLE::BINARY);
  assert(isotable.NumIsosurfaceVertices() == isotable.Polytope().NumEdges());
  ...
  for (long j = 0; j < num_iso_vertices; j++) {
    ...
    IJKMCUBE_TABLE::EDGE_INDEX ie = isotable.IsosurfaceVertex(j).Face();
    int iv0 = isotable.Polytope().EdgeEndpoint(ie, 0);
    int iv1 = isotable.Polytope().EdgeEndpoint(ie, 1);

    int d = 0;
    while (d < dimension && 
           isotable.Polytope().VertexCoord(iv0, d) ==
           isotable.Polytope().VertexCoord(iv1, d)) { d++; };
    ...
    for (int d2 = 0; d2 < dimension; d2++) {
      if (d2 != d && 
          isotable.Polytope().VertexCoord(iv0, d2) !=
          isotable.Polytope().VertexCoord(iv1, d2)) { 
        error.AddMessage("Illegal cube edge ", ie, ".");
        error.AddMessage("Endpoints ", iv0, " and ", iv1, 
    ...
    if (isotable.Polytope().VertexCoord(iv0, d) >
        isotable.Polytope().VertexCoord(iv1, d)) { std::swap(iv0, iv1); };
  ...
  //               isotable.NumIsosurfaceVertices()
{
  ...
  assert(vertex_increment != NULL);
  assert(increment != NULL);
  assert(isotable.Encoding() == IJKMCUBE_TABLE::BASE3);
  assert(isotable.NumIsosurfaceVertices() == 
         isotable.Polytope().NumEdges()+isotable.Polytope().NumVertices());
...
// Compute increment to add to index of current voxel
//   to get facet index.
// Precondition: Polytope is a cube.
// Precondition: array increment is allocated with size 
//               at least poly.NumFacets();
void IJKMCUBE::compute_facet_increment
(const ISOTABLE_POLY_BASE & poly, const AXIS_SIZE_TYPE * axis_size,
 VERTEX_INDEX * facet_increment)
{
  ...
  IJK::ARRAY<int> orthogonal_axis(numf);
  IJK::ARRAY<int> axis_intercept(numf);
  IJK::ARRAY<int> num_orthogonal(dimension);  // number of facets orthogonal to dimension
  IJK::ARRAY<int> facet0(dimension);
  IJK::ARRAY<int> facet1(dimension);
  ...
  for (int jf = 0; jf < poly.NumFacets(); jf++) {
    ...
    for (int d = 0; d < dimension; d++) {
      ...
      if (!coord_is_set) {
        error.AddMessage("No vertices lie on polytope facet ", jf, ".");
        throw error;
      }
    }

    if (!is_orthogonal_axis_set) {
      error.AddMessage("Unable to find axis orthogonal to polytope facet ", 
                       jf, ".");
      throw error;
    }
  }
...
namespace {
  ...
  (const ISOTABLE_POLY_BASE & poly, const int d,
   int & minc, int & maxc)
  {
    minc = maxc = 0;
...
// poly = isosurface table polytope.
//   Precondition: Polytope is a cube.
...
(const ISOTABLE_POLY_BASE & poly, const int orth_dir, const int side,
 const AXIS_SIZE_TYPE * axis_size, 
 VERTEX_INDEX * facet_vertex_increment, VERTEX_INDEX * cube_vertex_index)
{
  const int dimension = poly.Dimension();
  IJK::ARRAY<VERTEX_INDEX> axis_increment(dimension);
  IJK::ARRAY<int> minc(dimension);
  IJK::ARRAY<int> maxc(dimension);
...
// Return file name prefix of isosurface lookup table  
//   for given type, dimension and polytope.
std::string IJKMCUBE::get_isotable_filename_prefix
(const int dimension, const char * poly_name, const ISOTABLE_TYPE isotable_type)
{
  PROCEDURE_ERROR error("get_isotable_filename_prefix");
  std::string prefix;
  ...
  prefix += "." + std::string(poly_name);

  return prefix;
}


// Return file name suffix of isosurface lookup table  
//   for given type, dimension and polytope.
std::string IJKMCUBE::get_isotable_filename_suffix
(const int dimension, const char * poly_name, const ISOTABLE_TYPE isotable_type)
{
  std::ostringstream dimension_stringstream;
  dimension_stringstream << dimension;
  const std::string suffix = dimension_stringstream.str() + "D.xit";
  return suffix;
}
...
(const ISOSURFACE_TABLE & isotable, 
 const IJKMCUBE_TABLE::ENCODING encoding, ERROR & error)
{
  if (isotable.Encoding() != encoding) {
    error.AddMessage("Illegal isosurface table encoding.");
    if (encoding == IJKMCUBE_TABLE::BINARY)
      { error.AddMessage("  BINARY encoding required."); }
    else if (encoding == IJKMCUBE_TABLE::BASE3)
 ...
 ISOSURFACE_TOPOLOGY isosurface_topology, IJK::ERROR & error)
{
  ...
  if (numv != cube_isotable.Polytope().NumVertices()) {
    error.AddMessage("Isotable polytope is not a cube.");
    return(false);
  }
