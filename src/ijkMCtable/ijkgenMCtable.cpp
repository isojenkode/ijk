...
/*
  ...
  Copyright (C) 2001-2025 Rephael Wenger
...
void generate_edge_groups_isosurface_table(ISOSURFACE_TABLE & isotable)
{
  ...
  // *** SHOULD REALLY USE THE polytope (cube) in isotable here.
  // *** If the generated cube is different from isotable.Poly(),
  //     the output will be nonsense.
  ISOTABLE_CUBE_3D cube;
  if (use_cube_lex_order)
    { cube.GenCube3D(); }
  else
    { cube.GenCubeOrderA3D(); }
    ...
    for (int is = 0; is < num_simplices; is++) {
      for (int j = 0; j < isotable.NumVerticesPerSimplex(); j++) {
        ...
        if (prism_ie < nume) {
          ...
          error.AddMessage("Programming error. Illegal prism edge ", prism_ie,
                           " in prism isosurface vertex list");
          throw error;
        }
        isotable.SetSimplexVertex(it, is, j, ie);
...
void usage_msg()
{
  ...
  cout << "    [-ivol | -nep | -edge_groups | -chull] [-V <n>]" << endl;
...
void help_msg()
{
  ...
  cout << "        See \"Edge Groups:"
       ...
       << endl;
  cout << "  -chull: Use isosurface lookup table based solely on convex hull"
       << endl
       << "        triangulation." << endl;
  ...
  cout << "      Isosurface polytopes will not be consistently oriented."
       << endl;
  cout << "  -lex_order: Generate cube edges in lexicographic order."
       << endl;
  cout << "  -orderA:    Generate cube edges ordered by direction."
       << endl;
