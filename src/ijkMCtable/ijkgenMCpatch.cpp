...
namespace {
  ...
  (const std::vector<ISOSURFACE_VERTEX_INDEX> & boundary_edge_vertex_list,
   const unsigned int expected_num_edges,
   IJK::ERROR & error)
  {
 ...
 int * temp_plist, double * temp_pcoord)
{
  ...
  const unsigned int NUM_VERT_PER_TRIANGLE(3);
  const int NUM_VERT_PER_EDGE(2);
  ...
  */
  ...
  if (cube.Shape() != CUBE) {
    error.AddMessage("Programming error. Isotable polytope is not a cube.");
    error.AddMessage("  Routine get_isopatch_edge_groups expects a cube.");
    error.AddMessage("  Polytope is a ", cube.ShapeString(), ".");
    throw error;
  }
