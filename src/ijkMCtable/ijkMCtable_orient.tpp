...
namespace IJKMCUBE_TABLE {
  ...
  protected:
    ...
    (const TABLE_INDEX table_index,
     const ISOSURFACE_VERTEX_INDEX * simplex_vertex_list,
     const unsigned int num_simplices);
  ...
  namespace {
    ...
    void _swap_with_last(const unsigned int i, std::vector<ETYPE> & a)
    {      
      if (i+1 < a.size()) {
     ...
     const bool flag_verbose, const _NTYPE output_trigger)
    {
      ...
      typedef std::vector<TABLE_INDEX>::size_type SIZE_TYPE;
  ...
  (const TABLE_INDEX table_index,
   ...
   const unsigned int num_simplices)
  {
    typedef typename std::vector<_ITYPE>::size_type SIZE_TYPE;
    ...
    for (SIZE_TYPE isimplex = 0; isimplex < simplex_component.size();
         isimplex++) {
